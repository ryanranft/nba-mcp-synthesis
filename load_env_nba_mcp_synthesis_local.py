        ...
        if 'GOOGLE_API_KEY_NBA_MCP_SYNTHESIS_DEVELOPMENT' in self.secrets:
            try:
                response = requests.get(
                    ...
                    params={'key': self.secrets['GOOGLE_API_KEY_NBA_MCP_SYNTHESIS_DEVELOPMENT']},
        ...
        if 'OPENAI_API_KEY_NBA_MCP_SYNTHESIS_DEVELOPMENT' in self.secrets:
            try:
                response = requests.get(
                    ...
                    headers={'Authorization': f'Bearer {self.secrets["OPENAI_API_KEY_NBA_MCP_SYNTHESIS_DEVELOPMENT"]}'},
                    timeout=3
                )
...
class SlackNotifier:
    ...
    def __init__(self, webhook_url: Optional[str] = None):
        self.webhook_url = webhook_url or os.getenv('SLACK_WEBHOOK_URL_NBA_MCP_SYNTHESIS_DEVELOPMENT')
...
def load_local_secrets() -> Dict[str, str]:
    ...
    secrets_dir = Path("/Users/ryanranft/Desktop/++/big_cat_bets_assets/sports_assets/big_cat_bets_simulators/NBA/nba-mcp-synthesis/.env.nba_mcp_synthesis.development")
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.info(f"Development secrets loading completed in {execution_time:.2f} seconds")
