...
class TestConcurrentLoadScenarios:
    ...
    def test_concurrent_model_training(self, sample_player_data, test_helper):
        ...
        assert (
            total_time < 10.0
        ), f"Concurrent training took too long: {total_time:.2f}s"
    ...
    def test_concurrent_data_validation(self, large_player_dataset, test_helper):
        ...
        def validate_data_batch(data_batch, batch_id):
            ...
            validations = {
                ...
                "has_negatives": (data_batch.select_dtypes(include=[np.number]) < 0)
                .any()
                .any(),
                "valid": True,
            }
        ...
        assert (
            total_time < 5.0
        ), f"Concurrent validation took too long: {total_time:.2f}s"
    ...
    def test_large_dataset_aggregation_performance(self, test_helper):
        ...
        def aggregate_large_dataset(data):
            ...
            results["player_avg"] = data.groupby("player_id").agg(
                {"points": "mean", "assists": "mean", "rebounds": "mean"}
            )
    ...
    def test_cache_performance_high_load(self, test_helper):
        ...
        for i in range(num_operations):
            if i < num_unique_keys:
                ...
                if i % 10 == 0:
                    operations.append(
                        ("set", f"key_{i % num_unique_keys}", f"value_{i}")
                    )
                else:
...
def test_end_to_end_load_scenario(test_helper):
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        ...
        futures_pass1 = [
            executor.submit(process_with_cache, pid, data, cache) for pid in player_ids
        ]
    ...
    assert (
        cache_stats["hits"] >= num_concurrent * 0.8
    )  # At least 80% cache hits on second pass
