...
class TestEconometricDataValidationIntegration:
    ...
    def test_time_series_with_validation(self, time_series_data, test_helper):
        ...
        test_helper.assert_data_quality(
            pd.DataFrame({'points': time_series_data}),
            min_rows=100
        )
...
class TestEconometricPerformance:
    ...
    def test_time_series_initialization_performance(self, time_series_data, test_helper):
...
class TestEconometricDataPipeline:
    ...
    def test_econometric_pipeline_with_preprocessing(self, sample_player_data):
        ...
        # Step 3: Run analysis
        analyzer = TimeSeriesAnalyzer(
            data=pd.DataFrame({'points': ts_data}),
            target_column='points'
        )
...
@pytest.mark.agent_8
def test_comprehensive_agent_8_integration(sample_player_data, time_series_data, panel_data, test_helper):
    """
    ...
    assert workflow_state['ts_analysis_initialized'], f"Time series init should work: {workflow_state['errors']}"
    assert workflow_state['panel_analysis_initialized'], f"Panel data init should work: {workflow_state['errors']}"
    assert workflow_state['performance_monitored'], "Performance monitoring should track both analyses"
