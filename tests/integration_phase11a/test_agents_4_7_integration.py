...
class TestDataValidationTrainingIntegration:
    ...
    def test_validation_before_training(
            ...
            for col, range_spec in rules.get("ranges", {}).items():
                if col in data.columns:
                    ...
                    if "min" in range_spec and min_val < range_spec["min"]:
                        errors.append(
                            f"{col} below minimum: {min_val} < {range_spec['min']}"
                        )
                    if "max" in range_spec and max_val > range_spec["max"]:
                        errors.append(
                            f"{col} above maximum: {max_val} > {range_spec['max']}"
                        )
        ...
        def train_model_with_validation(data, rules):
            ...
            return {"model": model, "score": score, "training_samples": len(X_train)}
    ...
    def test_data_quality_metrics_logged(self, sample_player_data, test_helper):
        ...
        assert quality_metrics["before_training"]["row_count"] == len(
            sample_player_data
        )
...
class TestTrainingDeploymentIntegration:
    ...
    def test_model_versioning_workflow(self, sample_player_data, temp_output_dir):
        ...
        assert (
            model_registry["2.0"]["training_samples"]
            > model_registry["1.0"]["training_samples"]
        )
    ...
    def test_model_deployment_workflow(self, sample_player_data, temp_output_dir):
        ...
        deployment_state = {"staging": None, "production": None, "canary": None}
    ...
    def test_ab_testing_deployment(self, sample_player_data):
        ...
        def serve_with_ab_test(features, user_id):
            ...
            serving_log.append(
                {"user_id": user_id, "model": model_name, "prediction": prediction}
            )
        ...
        assert (
            abs(model_a_requests - model_b_requests) < 30
        ), "Traffic should be roughly split"
...
class TestValidationDeploymentIntegration:
    ...
    def test_rollback_on_validation_failure(self, sample_player_data):
        ...
        deployment = {"current": None, "previous": None, "rollback_count": 0}
...
class TestSystemIntegrationWorkflows:
    ...
    def test_end_to_end_training_deployment_workflow(
        self, sample_player_data, test_helper
    ):
        ...
        def engineer_features(data):
            ...
            data["efficiency"] = (data["points"] + data["assists"]) / (
                data["minutes"] + 0.1
            )
    ...
    def test_parallel_model_training_workflow(self, sample_player_data):
        ...
        def train_single_model(model_config):
            ...
            return {"model_name": model_config["name"], "model": model, "score": score}
        ...
        with ThreadPoolExecutor(max_workers=3) as executor:
            futures = [
                executor.submit(train_single_model, config) for config in model_configs
            ]
        ...
        assert set(r["model_name"] for r in results) == {
            "linear",
            "rf_small",
            "rf_medium",
        }
...
@pytest.mark.agents_4_7
def test_comprehensive_agent_4_7_workflow(
    sample_player_data, validation_rules, temp_output_dir, test_helper
):
    """
    ...
    assert (
        len(pipeline_state["errors"]) == 0
    ), f"No errors expected: {pipeline_state['errors']}"
