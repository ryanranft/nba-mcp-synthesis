...
async def run_performance_benchmarks():
    ...
    from tests.benchmarks.test_performance import TestPerformanceBenchmarks, TestScalabilityBenchmarks
...
def generate_test_report(results: Dict[str, Any]):
    ...
    if perf_results:
        ...
        passed_benchmarks = sum(1 for result in benchmarks.values() if result.get("passed", False))
        ...
        print(f"\nScalability Tests:")
        print(f"  Large Dataset: {'✓' if large_dataset.get('passed', False) else '✗'} ({large_dataset.get('dataset_size', 0)} records)")
        print(f"  Complex Formulas: {'✓' if complex_formula.get('passed', False) else '✗'} ({complex_formula.get('formulas_tested', 0)} formulas)")

    # Overall success
    overall_success = (
        unit_results.get("success", False) and
        integration_results.get("success", False) and
        (perf_results.get("benchmarks", {}).get("passed", True) if perf_results else True)
    )
