...
async def test_ab_framework():
    ...
    try:
        ...
        logger.info(
            f"✓ Initialized framework with results dir: {framework.results_dir}"
        )
...
async def main():
    ...
    if failed == 0:
        ...
        logger.info("\nNext steps:")
        logger.info(
            "1. Run A/B test: python scripts/ab_testing_framework.py --test gemini-vs-claude --books 2"
        )
        logger.info(
            "2. Discover books: python scripts/smart_book_discovery.py --scan-repos --dry-run"
        )
