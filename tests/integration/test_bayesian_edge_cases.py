...
async def test_bayesian_with_outliers(mock_context):
    ...
    params = BayesianLinearRegressionParams(
        data=data, formula="y ~ x1 + x2", n_samples=1000
    )
    ...
    assert result.posterior_mean["x1"] > 0
    assert result.posterior_mean["x2"] > 0
...
async def test_bayesian_convergence_diagnostics(mock_context):
    ...
    params = BayesianLinearRegressionParams(
        data=data, formula="y ~ x", n_samples=2000  # Sufficient for convergence
    )
    ...
    assert 1.5 < result.posterior_mean["x"] < 2.5
...
async def test_bayesian_with_small_sample(mock_context):
    ...
    params = BayesianLinearRegressionParams(data=data, formula="y ~ x", n_samples=1000)

    result = await bayesian_linear_regression(params, mock_context)

    assert result.success
    # With small sample, credible intervals should be wider
    x_interval = result.credible_intervals["x"]
    interval_width = x_interval["upper"] - x_interval["lower"]
...
async def test_bayesian_multicollinearity(mock_context):
    ...
    total_effect = result.posterior_mean["x1"] + result.posterior_mean["x2"]
...
@pytest.mark.integration
@pytest.mark.skip(
    reason="Intercept-only models (y ~ 1) not currently supported by tool"
)
async def test_bayesian_with_intercept_only(mock_context):
    ...
    params = BayesianLinearRegressionParams(
        data=data, formula="y ~ 1", n_samples=1000  # Intercept only
    )
    ...
    if result.success:
        ...
        assert "Intercept" in result.posterior_mean
        assert 13 < result.posterior_mean["Intercept"] < 17
...
async def test_bayesian_with_multiple_predictors(mock_context):
    ...
    params = BayesianLinearRegressionParams(
        data=data, formula="y ~ x1 + x2 + x3", n_samples=1500
    )
    ...
    assert 1.5 < result.posterior_mean["x1"] < 2.5
    assert 2.5 < result.posterior_mean["x2"] < 3.5
    assert 0.0 < result.posterior_mean["x3"] < 1.0
...
@pytest.mark.asyncio
@pytest.mark.integration
@pytest.mark.skip(reason="Interaction syntax (x1:x2) not currently supported by tool")
async def test_bayesian_with_categorical_interaction(mock_context):
    ...
    params = BayesianLinearRegressionParams(
        data=data, formula="y ~ x1 + x2 + x1:x2", n_samples=1500  # Include interaction
    )
    ...
    if result.success:
        ...
        assert "x1" in result.posterior_mean
        assert "x2" in result.posterior_mean
...
async def test_bayesian_with_zero_coefficient(mock_context):
    ...
    x2_interval = result.credible_intervals["x2"]
    assert (
        x2_interval["lower"] < 0 < x2_interval["upper"]
    )  # Zero should be in the interval
