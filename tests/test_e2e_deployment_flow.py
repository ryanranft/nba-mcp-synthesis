...
async def mock_e2e_deployment_flow(
    ...
    try:
        ...
        if config.get('create_prs'):
            ...
            result['pr_url'] = f"https://github.com/owner/repo/pull/{result['pr_number']}"
...
class TestE2EDeploymentFlow:
    ...
    async def test_03_synthesis_failure_rollback(
        ...
        assert 'error_message' in result
        assert 'timeout' in result['error_message'].lower() or 'synthesis' in result['error_message'].lower()
    ...
    async def test_06_git_workflow_failure(
        ...
        assert 'push failed' in result['error_message'].lower() or 'network' in result['error_message'].lower()
    ...
    async def test_07_pr_creation_failure(
        ...
        assert 'rate limit' in result['pr_creation_error'].lower() or 'github' in result['pr_creation_error'].lower()
    ...
    async def test_08_concurrent_deployments(
        ...
        branches = [r['branch_name'] for r in results if not isinstance(r, Exception) and r.get('branch_created')]
    ...
    async def test_10_full_integration_real_apis(
        ...
        # This would use the real deployment flow function
        # For now, we skip this test unless explicitly enabled
        pytest.skip("Integration test - requires real API keys and costs money")

        # In production:
        # result = await real_e2e_deployment_flow(
        #     book_path=sample_book_path,
        #     config=deployment_config
        # )
        #
