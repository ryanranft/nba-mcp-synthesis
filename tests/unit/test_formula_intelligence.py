...
class TestFormulaIntelligence(unittest.TestCase):
    ...
    def test_formula_type_identification(self):
        ...
        ts_type, confidence = self.intelligence.identify_formula_type(self.test_formulas["true_shooting"])
        self.assertEqual(ts_type, FormulaType.EFFICIENCY)
        self.assertGreater(confidence, 0.7)
        
        efg_type, confidence = self.intelligence.identify_formula_type(self.test_formulas["effective_fg"])
        self.assertEqual(efg_type, FormulaType.EFFICIENCY)
        self.assertGreater(confidence, 0.7)
        
        # Test rate formulas
        usage_type, confidence = self.intelligence.identify_formula_type(self.test_formulas["usage_rate"])
        self.assertEqual(usage_type, FormulaType.RATE)
        self.assertGreater(confidence, 0.7)
        
        ast_type, confidence = self.intelligence.identify_formula_type(self.test_formulas["assist_percentage"])
        self.assertEqual(ast_type, FormulaType.RATE)
        self.assertGreater(confidence, 0.7)
        
        # Test composite formulas
        per_type, confidence = self.intelligence.identify_formula_type(self.test_formulas["per"])
        self.assertEqual(per_type, FormulaType.COMPOSITE)
        self.assertGreater(confidence, 0.7)
        
        # Test differential formulas
        net_type, confidence = self.intelligence.identify_formula_type(self.test_formulas["net_rating"])
    ...
    def test_tool_suggestions(self):
        ...
        ts_suggestions = self.intelligence.suggest_tools(self.test_formulas["true_shooting"])
        ...
        usage_suggestions = self.intelligence.suggest_tools(self.test_formulas["usage_rate"])
    ...
    def test_variable_mapping(self):
        ...
        ts_mapping = self.intelligence.map_variables(self.test_formulas["true_shooting"])
        ...
        usage_mapping = self.intelligence.map_variables(self.test_formulas["usage_rate"])
    ...
    def test_unit_validation(self):
        ...
        usage_units = self.intelligence.validate_units({
            ...
            "variables": {
                "FGA": "attempts", "FTA": "attempts", "TOV": "turnovers",
                "MP": "minutes", "TM_MP": "minutes", "TM_FGA": "attempts",
                "TM_FTA": "attempts", "TM_TOV": "turnovers"
            }
        })
    ...
    def test_comprehensive_analysis(self):
        ...
        analysis = self.intelligence.analyze_comprehensive(self.test_formulas["true_shooting"])
...
class TestEdgeCases(unittest.TestCase):
    ...
    def test_complex_nested_formula(self):
        ...
        complex_formula = "((PTS + AST) * (FGM / FGA)) / (MP / 48) + (STL + BLK) / (TOV + 1)"
...
class TestIntegration(unittest.TestCase):
    ...
    def test_sports_context_integration(self):
        ...
        for formula in sports_formulas:
            ...
            self.assertIsInstance(analysis["formula_type"], FormulaType)
            self.assertGreater(analysis["confidence"], 0.3)  # Should have some confidence
            
            # Should provide sports-specific recommendations
            recommendations = self.intelligence.get_recommendations(formula, "basketball")
            self.assertIsInstance(recommendations, dict)
            self.assertIn("best_practices", recommendations)
