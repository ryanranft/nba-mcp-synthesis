...
class TestAssertions:
    ...
    @staticmethod
    def assert_performance_within_threshold(actual_time: float, tool_name: str, message: str = None):
    ...
    @staticmethod
    def assert_memory_within_limits(current_mb: float, peak_mb: float, message: str = None):
        ...
        if current_mb >= max_current:
            errors.append(f"Current memory {current_mb:.2f}MB exceeds limit {max_current}MB")
        if peak_mb >= max_peak:
    ...
    @staticmethod
    def assert_result_within_tolerance(actual: float, expected: float, tolerance: float = 0.01, message: str = None):
