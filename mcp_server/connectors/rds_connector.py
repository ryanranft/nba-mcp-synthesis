...
class RDSConnector:
    ...
    async def execute_query(
        ...
        result = await loop.run_in_executor(
            None, self._execute_sync, query, params, fetch_all, max_rows
        )
        return result

    def _execute_sync(
        self, query: str, params: Optional[tuple], fetch_all: bool, max_rows: int
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            with conn.cursor() as cursor:
                ...
                if (
                    ...
                    and "LIMIT" not in query_stripped
                    and "FROM" in query_stripped
                ):  # Only add LIMIT if there's a FROM clause
    ...
    async def get_table_statistics(self, table_name: str) -> Dict[str, Any]:
        ...
        size_result = await self.execute_query(
            size_query, (table_name, table_name, table_name)
        )
        ...
        if all(
            [count_result["success"], size_result["success"], index_result["success"]]
        ):
            return {
                "success": True,
                "table_name": table_name,
        ...
        return {"success": False, "error": "Failed to get complete statistics"}
