...
class OddsDatabaseConnector:
    ...
    def __init__(self, db_config: Optional[Dict[str, Any]] = None):
        ...
        if not all(
            ...
            raise ValueError(
                "Database configuration incomplete. Ensure secrets are loaded."
            )
    ...
    def get_latest_odds_for_game(
        ...
        try:
            with conn.cursor() as cur:
                ...
                results = cur.fetchall()
                logger.debug(
                    f"Found {len(results)} odds for event {event_id}, market {market}"
                )
                return [dict(row) for row in results]
        except Exception as e:
    ...
    def get_all_odds_today(
        self, market: str = "h2h", top_bookmakers: Optional[List[str]] = None
        ...
        for game in games:
            ...
            odds = self.get_latest_odds_for_game(
                event_id, market=market, bookmaker_filter=top_bookmakers
            )
            if odds:
                all_odds[event_id] = {"game_info": game, "odds": odds}

        logger.info(f"Retrieved odds for {len(all_odds)} games")
        return all_odds

    def get_best_odds_by_bookmaker(
        self, event_id: str, market: str = "h2h"
    ...
    def get_consensus_odds(
        ...
        """
        odds = self.get_latest_odds_for_game(
            event_id, market, bookmaker_filter=["draftkings", "fanduel", "betmgm"]
        )
    ...
    def map_game_to_event_id(
        ...
        try:
            with conn.cursor() as cur:
                ...
                home_partial = (
                    f"%{home_team.split()[-1]}%"  # Last word (e.g., "Lakers")
                )
                away_partial = f"%{away_team.split()[-1]}%"

                cur.execute(
                    query_partial,
                    (game_date, home_team, home_partial, away_team, away_partial),
                )
                result = cur.fetchone()

                if result:
                    logger.info(
                        f"Mapped {away_team} @ {home_team} to event {result['event_id']}"
                    )
                    return result["event_id"]
                else:
                    logger.warning(
                        f"No event_id found for {away_team} @ {home_team} on {game_date}"
                    )
                    return None
...
if __name__ == "__main__":
    ...
    with OddsDatabaseConnector() as connector:
        ...
        for game in games[:3]:
            print(
                f"  {game['away_team']} @ {game['home_team']} - {game['commence_time']}"
            )
