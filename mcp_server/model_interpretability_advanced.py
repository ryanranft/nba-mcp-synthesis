...
class CounterfactualGenerator:
    ...
    def _simulate_prediction(self, features: Dict[str, float]) -> float:
        ...
        weights = {"ppg": 0.3, "rpg": 0.2, "apg": 0.2, "fg_percent": 0.15, "mpg": 0.15}
...
class AnchorExplainer:
    ...
    def explain_with_anchors(
        self, features: Dict[str, float], prediction: float, feature_names: List[str]
    ) -> AnchorExplanation:
        """
    ...
    def find_prototypes(
        ...
        return {
            ...
            "average_distance": round(
                sum(p["distance"] for p in prototypes) / n_prototypes, 2
            ),
    ...
    def analyze_decision_boundary(
        ...
        return {
            ...
            "samples": [
                ...
                for fv, p in zip(
                    feature_values[::5], predictions[::5]
                )  # Every 5th point
            ],
        }
...
class AdvancedInterpretability:
    ...
    def explain_prediction(
        ...
        counterfactual = self.counterfactual_gen.generate_counterfactual(
            features, prediction, 0.8, feature_ranges  # Desired: All-Star level
        )

        # Anchor
        anchor = self.anchor_explainer.explain_with_anchors(
            features, prediction, list(features.keys())
        )
        ...
        if training_data and training_labels:
            prototypes = self.prototype_explainer.find_prototypes(
                features, training_data, training_labels
            )
        ...
        return {
            ...
            "counterfactual": {
                "changes_needed": {
                    k: round(v, 2) for k, v in counterfactual.changes_needed.items()
                },
...
if __name__ == "__main__":
    ...
    explanation = interp.explain_prediction(
        player_features, current_prediction, training_data, training_labels
    )
    ...
    if explanation["prototypes"]:
        ...
        for proto in explanation["prototypes"]["prototypes"]:
            print(
                f"  Similarity: {proto['similarity']} - Label: {'All-Star' if proto['label'] else 'Not All-Star'}"
            )

    # Decision boundary
    print(f"\n--- Decision Boundary Analysis ---")
    boundary = interp.behavior_analyzer.analyze_decision_boundary(
        "ppg", (10, 30), {k: v for k, v in player_features.items() if k != "ppg"}
    )
    print(f"Feature: {boundary['feature']}")
    print(f"Sensitivity: {boundary['sensitivity']}")
    print(
        f"Prediction range: {boundary['prediction_range'][0]:.2f} to {boundary['prediction_range'][1]:.2f}"
    )
