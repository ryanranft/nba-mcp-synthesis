...
class UnifiedSecretsManager:
    ...
    def load_secrets_hierarchical(
        ...
        for level_name, level_path in hierarchy_levels.items():
            if level_path.exists():
                level_secrets = self._load_secrets_from_level(level_path, level_name, context)
                ...
                logger.info(f"Loaded {len(level_secrets)} secrets from {level_name} level")
            else:
                logger.debug(f"Skipping {level_name} level - path not found: {level_path}")
        ...
        if result.secrets_loaded == 0 and os.getenv('USE_AWS_SECRETS_MANAGER', '').lower() == 'true':
            logger.warning("No local secrets found, attempting AWS Secrets Manager fallback")
            ...
            if aws_secrets:
                ...
                result.errors.append("No secrets found locally or in AWS Secrets Manager")
    ...
    def _get_hierarchy_levels(self, project: str, sport: str, context: str) -> Dict[str, Path]:
        ...
        return {
            "global": self.base_path / "sports_assets" / "big_cat_bets_global" / f".env.global.{context_dir}",
            "simulator_global": self.base_path / "sports_assets" / "big_cat_bets_simulators" / "_config_placeholder" / f".env.simulator_global.{context_dir}",
            "sport": self.base_path / "sports_assets" / "big_cat_bets_simulators" / sport / "_config_placeholder" / f".env.{sport.lower()}_sport.{context_dir}",
            "project_group": self.base_path / "sports_assets" / "big_cat_bets_simulators" / sport / f"{project_dir}_global" / f".env.{project_dir}_global.{context_dir}",
            "project": self.base_path / "sports_assets" / "big_cat_bets_simulators" / sport / project / f".env.{project_dir}.{context_dir}"
        }

    def _load_secrets_from_level(self, level_path: Path, level_name: str, context: str) -> Dict[str, str]:
            ...
            try:
                with open(env_file, 'r') as f:
                    ...
                    if self._is_valid_naming_convention(secret_name):
                        ...
                        logger.warning(f"Invalid naming convention for {secret_name} in {level_name}")
    ...
    def _is_valid_naming_convention(self, secret_name: str) -> bool:
        ...
        if len(parts) == 3 and 'GLOBAL' in parts:
            has_service = any(part in ['GOOGLE', 'ANTHROPIC', 'OPENAI', 'DEEPSEEK', 'SLACK', 'LINEAR', 'DB', 'AWS', 'REDIS', 'RDS', 'S3', 'GLUE', 'NBA', 'SPORTSDATA', 'TIMEZONE'] for part in parts)
            resource_type_patterns = ['API_KEY', 'SECRET_KEY', 'ACCESS_KEY', 'PASSWORD', 'TOKEN', 'WEBHOOK_URL', 'HOST', 'PORT', 'DATABASE', 'BUCKET', 'USERNAME', 'REGION', 'TIMEZONE']
            has_resource_type = any(pattern in secret_name for pattern in resource_type_patterns)
            has_context = any(part in ['WORKFLOW', 'DEVELOPMENT', 'TEST', 'PRODUCTION'] for part in parts)
...
def load_secrets_hierarchical(project: str = None, sport: str = None, context: str = None) -> bool:
    """
...
def get_database_config() -> Dict[str, Any]:
    ...
    return {
        'host': get_secret('DB_HOST') or get_secret('DB_HOST_NBA_MCP_SYNTHESIS_WORKFLOW'),
        'port': get_secret('DB_PORT') or get_secret('DB_PORT_NBA_MCP_SYNTHESIS_WORKFLOW'),
        'database': get_secret('DB_NAME') or get_secret('DB_NAME_NBA_MCP_SYNTHESIS_WORKFLOW'),
        'user': get_secret('DB_USER') or get_secret('DB_USER_NBA_MCP_SYNTHESIS_WORKFLOW'),
        'password': get_secret('DB_PASSWORD') or get_secret('DB_PASSWORD_NBA_MCP_SYNTHESIS_WORKFLOW'),
    }

def get_s3_bucket() -> str:
    """Get S3 bucket name (backward compatibility)"""
    return get_secret('S3_BUCKET') or get_secret('S3_BUCKET_NBA_MCP_SYNTHESIS_WORKFLOW') or "nba-mcp-books-20251011"
