...
class FunnelAnalyzer:
    ...
    def analyze_funnel(
        ...
        for step in steps[1:]:
            ...
            conversion_rate = (len(completed_users) / len(users_in_funnel) * 100) if users_in_funnel else 0
        ...
        for i in range(len(step_counts) - 1):
            ...
            drop_off_rate = (drop_off / step_counts[i] * 100) if step_counts[i] > 0 else 0
            drop_offs.append({
...
class FeatureUsageTracker:
    ...
    def get_feature_usage(
        ...
        return {
            ...
            'avg_uses_per_user': round(len(feature_events) / len(unique_users), 2) if unique_users else 0,
            'first_use': first_use.isoformat() if first_use else None,
            'last_use': last_use.isoformat() if last_use else None,
            'power_users': sorted(
                [{'user_id': uid, 'count': count} for uid, count in usage_by_user.items()],
    ...
    def get_adoption_rate(
        ...
        adoption_rate = (usage['unique_users'] / total_users * 100) if total_users > 0 else 0
...
class BusinessAnalytics:
    ...
    def get_dashboard_metrics(self, period_days: int = 30) -> Dict[str, Any]:
        ...
        # Unique users
        unique_users = set(
            e.user_id for e in self.collector.events
        ...
        return {
            ...
            'unique_users': len(unique_users),
            'events_per_user': round(total_events / len(unique_users), 2) if unique_users else 0,
            'events_by_type': events_by_type,
            'total_conversions': conversions,
            'conversion_rate': round(conversions / len(unique_users) * 100, 2) if unique_users else 0
        }
...
if __name__ == "__main__":
    ...
    for i, user in enumerate(users):
        ...
        # 80% use feature
        if i < 80:
            analytics.track(user, EventType.FEATURE_USED, {'feature_name': 'player_stats'})
