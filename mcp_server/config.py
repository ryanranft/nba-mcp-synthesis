...
class MCPConfig:
    ...
    allowed_sql_keywords: List[str] = field(
        default_factory=lambda: ["SELECT", "EXPLAIN", "SHOW", "DESCRIBE", "WITH"]
    )
    ...
    def from_env(cls) -> "MCPConfig":
        ...
        return cls(
            ...
            glue_database=(
                ...
                or os.getenv("GLUE_DATABASE", "nba_raw_data")
            ),  # Fallback to old name
            glue_region=(
            ...
            notify_on_success=os.getenv("SLACK_NOTIFY_SUCCESS", "false").lower()
            == "true",
            # Limits
    ...
    def validate(self) -> List[str]:
        ...
        aws_access_key = (
            os.getenv("AWS_ACCESS_KEY_ID_NBA_MCP_SYNTHESIS_WORKFLOW")
            or os.getenv("AWS_ACCESS_KEY_ID_NBA_MCP_SYNTHESIS_DEVELOPMENT")
            or os.getenv("AWS_ACCESS_KEY_ID_NBA_MCP_SYNTHESIS_TEST")
            or os.getenv("AWS_ACCESS_KEY_ID")
        )  # Fallback to old name

        aws_secret_key = (
            os.getenv("AWS_SECRET_ACCESS_KEY_NBA_MCP_SYNTHESIS_WORKFLOW")
            or os.getenv("AWS_SECRET_ACCESS_KEY_NBA_MCP_SYNTHESIS_DEVELOPMENT")
            or os.getenv("AWS_SECRET_ACCESS_KEY_NBA_MCP_SYNTHESIS_TEST")
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            "s3": {"bucket": self.s3_bucket, "region": self.s3_region},
            "glue": {"database": self.glue_database, "region": self.glue_region},
            "project": {
