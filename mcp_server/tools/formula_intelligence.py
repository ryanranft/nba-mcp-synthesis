...
class FormulaIntelligence:
    ...
    def _build_variable_mappings(self) -> Dict[str, str]:
        ...
        return {
            ...
            # 3-Pointers
            '3PM': '3PM', '3-Pointers Made': '3PM', '3P Made': '3PM',
            '3PA': '3PA', '3-Pointers Attempted': '3PA', '3P Attempted': '3PA',
            '3P%': '3P%', '3-Point %': '3P%',

            # Free Throws
            'FTM': 'FTM', 'Free Throws Made': 'FTM', 'FT Made': 'FTM',
            'FTA': 'FTA', 'Free Throws Attempted': 'FTA', 'FT Attempted': 'FTA',
            'FT%': 'FT%', 'Free Throw %': 'FT%',

            # Other Stats
            'PTS': 'PTS', 'Points': 'PTS', 'Points Scored': 'PTS',
            'REB': 'REB', 'Rebounds': 'REB', 'Total Rebounds': 'REB',
            'OREB': 'OREB', 'Offensive Rebounds': 'OREB', 'Off Rebounds': 'OREB',
            'DREB': 'DREB', 'Defensive Rebounds': 'DREB', 'Def Rebounds': 'DREB',
            'AST': 'AST', 'Assists': 'AST',
            'STL': 'STL', 'Steals': 'STL',
            'BLK': 'BLK', 'Blocks': 'BLK',
            'TOV': 'TOV', 'Turnovers': 'TOV',
            'PF': 'PF', 'Personal Fouls': 'PF', 'Fouls': 'PF',
            'MP': 'MP', 'Minutes': 'MP', 'Minutes Played': 'MP',

            # Team Stats
            'TM_': 'TM_', 'Team ': 'TM_',
            'OPP_': 'OPP_', 'Opponent ': 'OPP_',

            # Advanced Metrics
            'PER': 'PER', 'Player Efficiency Rating': 'PER',
            'TS%': 'TS%', 'True Shooting %': 'TS%',
            'USG%': 'USG%', 'Usage Rate': 'USG%',
            'BPM': 'BPM', 'Box Plus/Minus': 'BPM',
            'VORP': 'VORP', 'Value Over Replacement': 'VORP',
            'WS': 'WS', 'Win Shares': 'WS',
            'PIE': 'PIE', 'Player Impact Estimate': 'PIE'
        }
    ...
    def suggest_tool(self, formula_str: str) -> List[ToolSuggestion]:
        ...
        if 'derivative' in formula_str.lower() or 'rate of change' in formula_str.lower():
    ...
    def validate_units(self, formula: Dict[str, Any]) -> bool:
        ...
        for var in percentage_vars:
            if var in variables:
                ...
                if isinstance(value, (int, float)):
                    if value > 1.0:
                        logger.warning(f"{var} value {value} appears to be percentage, should be decimal")
                        return False
        ...
        count_vars = ['FGM', 'FGA', 'FTM', 'FTA', '3PM', '3PA', 'PTS', 'REB', 'AST', 'STL', 'BLK']
    ...
    def analyze_formula(self, formula_str: str, variables: Optional[Dict[str, Any]] = None) -> FormulaAnalysis:
        """
    ...
    def _generate_insights(self, formula_str: str, formula_type: FormulaType, complexity_score: float) -> List[str]:
        """Generate insights about the formula"""
        insights = []

        # Type-specific insights
        if formula_type == FormulaType.EFFICIENCY:
            insights.append("This is an efficiency metric that measures shooting effectiveness")
            insights.append("Consider using True Shooting % for comprehensive shooting analysis")
            ...
            insights.append("This measures the difference between offensive and defensive performance")
            insights.append("Useful for evaluating overall team impact")

        # Complexity insights
        if complexity_score > 0.7:
            insights.append("This is a complex formula - consider breaking it into steps")
        elif complexity_score < 0.3:
            insights.append("This is a simple formula - easy to understand and calculate")

        # Variable insights
        if 'MP' in formula_str:
            insights.append("Formula includes minutes played - useful for rate calculations")

        if '3PM' in formula_str or '3PA' in formula_str:
            insights.append("Formula accounts for 3-pointers - modern basketball consideration")

        if 'TOV' in formula_str:
            insights.append("Formula includes turnovers - important for efficiency analysis")

        return insights

    def get_formula_recommendations(self, formula_str: str, context: str = "") -> Dict[str, Any]:
        """
        ...
        if analysis.complexity_score > 0.7:
            recommendations["recommendations"].append("Break complex formula into smaller components for analysis")

        if not analysis.unit_consistency:
            recommendations["recommendations"].append("Check and standardize units before calculation")

        if analysis.confidence < 0.5:
            recommendations["recommendations"].append("Verify formula accuracy with multiple sources")
...
def analyze_formula(formula_str: str, variables: Optional[Dict[str, Any]] = None) -> FormulaAnalysis:
