...
class IntelligentRecommendationEngine:
    ...
    def get_intelligent_recommendations(
        ...
        try:
            logger.info(
                f"Generating recommendations for context: {context.user_query[:50]}..."
            )
            ...
            sorted_recommendations = sorted(
                ...
                key=lambda x: (
                    x.confidence_score,
                    self._calculate_relevance_score(x, context),
                ),
                reverse=True,
            )
            ...
            return self._get_fallback_recommendations(
                context.analysis_type, max_recommendations
            )
    ...
    def _analyze_context(self, context: RecommendationContext) -> Dict[str, Any]:
        ...
        analysis = {
            ...
            "user_expertise": (
                context.user_context.expertise_level
                if context.user_context
                else UserExpertiseLevel.INTERMEDIATE
            ),
        ...
        if any(
            word in query_lower for word in ["shooting", "shot", "scoring", "points"]
        ):
            analysis["preferred_categories"].append("shooting")
        if any(
            word in query_lower
            for word in ["defense", "defensive", "defend", "steal", "block"]
        ):
            analysis["preferred_categories"].append("defensive")
        if any(
            word in query_lower for word in ["efficiency", "effective", "per", "rating"]
        ):
            analysis["preferred_categories"].append("efficiency")
        if any(
            word in query_lower for word in ["team", "teamwork", "lineup", "rotation"]
        ):
            analysis["preferred_categories"].append("team")
        if any(
            word in query_lower for word in ["advanced", "complex", "sophisticated"]
        ):
    ...
    def _get_semantic_recommendations(
        self, context: RecommendationContext, analysis: Dict[str, Any]
    ) -> List[FormulaRecommendation]:
        ...
        try:
            ...
            similarities = cosine_similarity(query_embedding, self.formula_embeddings)[
                0
            ]
            ...
            for idx in top_indices:
                if similarities[idx] > 0.1:  # Minimum similarity threshold
                    ...
                    recommendation = FormulaRecommendation(
                        ...
                        use_case=(
                            formula_info["use_cases"][0]
                            if formula_info["use_cases"]
    ...
    def _get_category_recommendations(
        ...
        if not target_categories:
            target_categories = (
                ...
                if context.analysis_type != "all"
                else ["efficiency", "shooting", "defensive"]
            )
        ...
        for formula_id, formula_info in self.formula_database.items():
            if formula_info["category"] in target_categories:
                ...
                confidence = (
                    0.8 if formula_info["category"] in target_categories else 0.5
                )
                ...
                if analysis["complexity_preference"] == formula_info["complexity"]:
                    ...
                    analysis["complexity_preference"] == "simple"
                    and formula_info["complexity"] == "complex"
                ):
                    confidence -= 0.2
    ...
    def _get_preference_recommendations(
        ...
        for formula_id, formula_info in self.formula_database.items():
            if any(
                pref_type in formula_info["name"].lower()
                or pref_type in formula_info["category"]
    ...
    def _deduplicate_recommendations(
        ...
        for rec in recommendations:
            if (
                rec.formula_id not in seen
                or seen[rec.formula_id].confidence_score < rec.confidence_score
            ):
    ...
    def _calculate_relevance_score(
        self, recommendation: FormulaRecommendation, context: RecommendationContext
    ) -> float:
        ...
        if (
            ...
            and recommendation.recommendation_type.value == context.analysis_type
        ):
            score += 0.3
        ...
        if context.user_context:
            if (
                context.user_context.expertise_level == UserExpertiseLevel.BEGINNER
                and recommendation.complexity_level == "simple"
            ):
                score += 0.2
            elif (
                context.user_context.expertise_level == UserExpertiseLevel.EXPERT
                and recommendation.complexity_level == "complex"
            ):
                score += 0.2
    ...
    def suggest_formulas_from_data(
        ...
        try:
            logger.info(
                f"Suggesting formulas for data with {len(available_variables)} variables"
            )
            ...
            sorted_suggestions = sorted(
                pattern_recs, key=lambda x: x.confidence_score, reverse=True
            )
    ...
    def analyze_user_context(
        ...
        try:
            ...
            analysis = {
                ...
                "complexity_preference": self._infer_complexity_preference(
                    user_query, user_expertise_level
                ),
                "preferred_categories": self._infer_preferred_categories(user_query),
                "session_patterns": (
                    self._analyze_session_patterns(session_history)
                    if session_history
                    else {}
                ),
    ...
    def _infer_analysis_type(self, query: str) -> str:
        ...
        if any(
            word in query_lower
            for word in ["shooting", "shot", "scoring", "points", "fg", "3pt"]
        ):
            return "shooting"
        elif any(
            word in query_lower
            ...
            word in query_lower
            for word in ["efficiency", "effective", "per", "rating", "win shares"]
        ):
            return "efficiency"
        elif any(
            word in query_lower for word in ["team", "teamwork", "lineup", "rotation"]
            ...
            word in query_lower
            for word in ["advanced", "complex", "sophisticated", "bpm", "vorp"]
        ):
            return "advanced"
        else:
            return "all"
    ...
    def _infer_complexity_preference(self, query: str, expertise_level: str) -> str:
        ...
        if any(
            word in query_lower
            for word in ["simple", "basic", "easy", "straightforward"]
        ):
            return "simple"
        elif any(
            word in query_lower
            for word in ["complex", "advanced", "sophisticated", "detailed"]
        ):
            return "complex"
    ...
    def _analyze_session_patterns(
        ...
        # Set patterns
        patterns["frequent_formulas"] = sorted(
            formula_counts.items(), key=lambda x: x[1], reverse=True
        )[:5]
    ...
    def _determine_recommendation_strategy(
        self, query: str, expertise_level: str
    ) -> str:
    ...
    def get_predictive_recommendations(
        ...
        try:
            logger.info(
                f"Generating predictive recommendations for {prediction_target}"
            )
            ...
            predictive_recs = [
                rec for rec in recommendations if rec.formula_id in predictive_formulas
            ]
    ...
    def detect_and_correct_errors(
        ...
        try:
            ...
            if "**" in formula_expression and "**" not in ["**2", "**3"]:
                ...
                analysis["correction_suggestions"].append(
                    "Consider using '^' or 'pow()' for exponentiation"
                )
            ...
            if input_values:
                try:
                    ...
                    if expected_result is not None:
                        error_magnitude = (
                            abs(result - expected_result) / abs(expected_result)
                        ...
                        if error_magnitude > error_tolerance:
                            ...
                            analysis["correction_suggestions"].append(
                                f"Result {result} differs from expected {expected_result}"
                            )
                        else:
                            analysis["confidence"] += 0.3
    ...
    def _get_semantic_recommendations(
        ...
        try:
            ...
            for formula_id, formula_info in self.formula_database.items():
                score = 0.0
                formula_text = (
                    f"{formula_info['name']} {formula_info['description']}".lower()
                )
        ...
        try:
            ...
            for formula_id, formula_info in self.formula_database.items():
                ...
                if (
                    ...
                    or (
                        user_expertise == UserExpertiseLevel.ADVANCED
                        and complexity in ["moderate", "complex"]
                    )
                    or (user_expertise == UserExpertiseLevel.EXPERT)
                ):
    ...
    def _calculate_relevance_score(
        ...
        if (
            ...
            and context.user_context.expertise_level == UserExpertiseLevel.BEGINNER
        ):
            if recommendation.complexity_level == "simple":
                score += 0.2
        elif (
            context.user_context
            and context.user_context.expertise_level == UserExpertiseLevel.EXPERT
        ):
    ...
    def _get_fallback_recommendations(
        self, analysis_type: str, max_recommendations: int
    ) -> List[FormulaRecommendation]:
...
def get_intelligent_recommendations(
    ...
    try:
        ...
        user_context = UserContext(
            preferred_formula_types=(
                user_preferences.get("preferred_types", []) if user_preferences else []
            ),
            expertise_level=(
                UserExpertiseLevel(
                    user_preferences.get("expertise_level", "intermediate")
                )
                if user_preferences
                else UserExpertiseLevel.INTERMEDIATE
            ),
        ...
        rec_context = RecommendationContext(
            user_query=context, analysis_type=analysis_type, user_context=user_context
        )
...
def analyze_user_context_for_recommendations(
    ...
    try:
        ...
        analysis = engine.analyze_user_context(
            user_query, session_history, current_analysis, user_expertise_level
        )
        ...
        return {
            ...
            "recommendations": {
                ...
                "complexity_preference": analysis.get(
                    "complexity_preference", "moderate"
                ),
                "preferred_categories": analysis.get("preferred_categories", []),
                "recommendation_strategy": analysis.get(
        ...
        return {"status": "error", "error": str(e), "context_analysis": {}}
...
def detect_and_correct_formula_errors(
    ...
    try:
        ...
        analysis = engine.detect_and_correct_errors(
            formula_expression, expected_result, input_values, error_tolerance
        )
