...
class InteractiveFormulaPlayground:
    ...
    def create_session(
        self, user_id: str, mode: PlaygroundMode, template_name: str = None
    ) -> PlaygroundSession:
        ...
        if template_name:
            template = next(
                (t for t in self.playground_templates if t["name"] == template_name),
                None,
            )
            if template:
    ...
    def add_formula_to_session(
        self, session_id: str, formula: str, description: str = ""
    ) -> Dict[str, Any]:
    ...
    def update_variables(
        self, session_id: str, variables: Dict[str, float]
    ) -> Dict[str, Any]:
    ...
    def calculate_formula_results(self, session_id: str) -> Dict[str, Any]:
        ...
        for i, formula_entry in enumerate(session.formulas):
            ...
            try:
                ...
                if formula in [
                    f["template"] for f in self.formula_builder.formula_templates
                ]:
                    template = next(
                        f
                        for f in self.formula_builder.formula_templates
                        if f["template"] == formula
                    )
                    result = self.formula_builder.create_formula_from_template(
                        template["name"], session.variables
                    )
                else:
                    # Use general formula calculation
                    result = self.formula_builder.get_formula_preview(
                        formula, session.variables
                    )
    ...
    def generate_visualizations(
        self, session_id: str, visualization_types: List[VisualizationType]
    ) -> Dict[str, Any]:
        ...
        for viz_type in visualization_types:
            if viz_type in self.visualization_tools:
                try:
                    ...
                    visualizations[viz_type.value] = {"error": str(e), "success": False}
        ...
        if session.formulas:
            for formula_entry in session.formulas:
                ...
                for related in related_formulas:
                    if not any(
                        (f if isinstance(f, str) else f["formula"]) == related
                        for f in session.formulas
                    ):
                        recommendations.append(
                            PlaygroundRecommendation(
                                type="formula",
    ...
    def get_shared_session(self, share_token: str) -> Dict[str, Any]:
        ...
        session = next(
            (s for s in self.sessions.values() if s.share_token == share_token), None
        )
    ...
    def create_experiment(
        self, session_id: str, experiment_name: str, description: str = ""
    ) -> Dict[str, Any]:
        ...
        visualizations = self.generate_visualizations(
            session_id, [VisualizationType.LATEX, VisualizationType.CHART]
        )
        ...
        if session.formulas:
            first_formula = (
                session.formulas[0]
                if isinstance(session.formulas[0], str)
                else session.formulas[0]["formula"]
            )
        ...
        return {"success": True, "experiment": asdict(experiment)}
    ...
    def _render_latex(self, session: PlaygroundSession) -> Dict[str, Any]:
        ...
        for formula_entry in session.formulas:
            ...
            try:
                ...
                latex_formulas.append({"formula": formula, "error": str(e)})

        return {"success": True, "type": "latex", "formulas": latex_formulas}
                ...
                table_data.append(
                    {
                        ...
                        "result": (
                            result.get("result", {}).get("calculated_value")
    ...
    def _get_related_formulas(self, formula: str) -> List[str]:
        ...
        if "PTS" in formula and "FGA" in formula:
            related.extend(["(FGM + 0.5 * 3PM) / FGA", "FGM / FGA"])  # eFG%  # FG%

        if "FGM" in formula and "FGA" in formula:
            related.extend(
                ["PTS / (2 * (FGA + 0.44 * FTA))", "3PM / 3PA"]  # TS%  # 3P%
            )
