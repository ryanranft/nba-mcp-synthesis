...
class CourtPlotter:
    ...
    def draw_court(
        ...
        ax.plot([0, 0], [0, self.COURT_LENGTH / 2 if self.half_court else self.COURT_LENGTH],
                color=color, linewidth=linewidth)
        ax.plot([self.COURT_WIDTH, self.COURT_WIDTH],
    ...
    def _draw_three_point_line(self, ax, color, linewidth):
        ...
        # Corners
        ax.plot([0, 3], [self.THREE_POINT_CORNER_HEIGHT, self.THREE_POINT_CORNER_HEIGHT],
                color=color, linewidth=linewidth)
        ax.plot([self.COURT_WIDTH - 3, self.COURT_WIDTH],
    ...
    def _draw_paint(self, ax, color, linewidth):
        ...
        # Paint edges
        ax.plot([left_x, left_x], [0, self.PAINT_LENGTH],
                color=color, linewidth=linewidth)
        ax.plot([right_x, right_x], [0, self.PAINT_LENGTH],
                color=color, linewidth=linewidth)

        # Free throw line
        ax.plot([left_x, right_x], [self.PAINT_LENGTH, self.PAINT_LENGTH],
                color=color, linewidth=linewidth)
    ...
    if home_positions:
        ...
        if show_labels:
            for p in home_positions:
                ax.text(p['x'], p['y'], p.get('player_id', '?'),
...
def plot_movement_path(
    ...
    # Mark start and end
    ax.scatter(x_coords[0], y_coords[0], c='green', marker='o', s=100, label='Start', zorder=10)
    ax.scatter(x_coords[-1], y_coords[-1], c='red', marker='s', s=100, label='End', zorder=10)
