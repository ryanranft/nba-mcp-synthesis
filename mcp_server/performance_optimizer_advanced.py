    ...
    def recommend_indexes(self, query: str) -> List[str]:
        ...
        if "JOIN" in query.upper():
            ...
            if join_match:
                ...
                recommendations.append(
                    f"CREATE INDEX idx_{column}_fk ON table_name ({column});"
                )
...
class CacheOptimizer:
    ...
    def recommend_cache_strategy(
        ...
        sorted_patterns = sorted(
            request_patterns.items(), key=lambda x: x[1], reverse=True
        )
        ...
        hot_key_coverage = (
            hot_key_requests / total_requests if total_requests > 0 else 0
        )
...
class ResourceOptimizer:
    ...
    def recommend_scaling(
        self, current_capacity: Dict[str, Any], projected_growth: float = 1.5
    ) -> List[Optimization]:
...
class AdvancedPerformanceOptimizer:
    ...
    def analyze_system_performance(
        ...
        recommendations = self._generate_recommendations(
            resource_analysis, query_analysis, cache_analysis, current_capacity
        )
    ...
    def _generate_recommendations(
        ...
        if query_analysis:
            total_optimization_potential = sum(
                q["optimization_potential"] for q in query_analysis
            )
        ...
        if resource_analysis.get("bottlenecks"):
            high_severity = [
                b for b in resource_analysis["bottlenecks"] if b["severity"] == "HIGH"
            ]
            if high_severity:
                recommendations.append(
                    {
...
if __name__ == "__main__":
    ...
    # Current capacity
    current_capacity = {"cpu_cores": 4, "memory_gb": 16, "instances": 2}
    ...
    analysis = optimizer.analyze_system_performance(
        metrics_history, slow_queries, cache_metrics, current_capacity
    )
    ...
    print(
        f"Memory: {summary['avg_memory_percent']}% avg, {summary['max_memory_percent']}% max"
    )
    ...
    for i, query in enumerate(analysis["query_analysis"][:2], 1):
        ...
        print(
            f"  Top recommendation: {query['recommendations'][0] if query['recommendations'] else 'None'}"
        )
