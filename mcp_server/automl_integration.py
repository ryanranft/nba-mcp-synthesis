...
class FeatureSelector:
    ...
    def select_features(
        ...
        for i, name in enumerate(feature_names):
            ...
            numerator = sum(
                (feature_values[j] - mean_x) * (y[j] - mean_y) for j in range(len(y))
            )
...
class HyperparameterTuner:
    ...
    def get_default_params(self, algorithm: Algorithm) -> Dict[str, Any]:
        ...
        defaults = {
            ...
            Algorithm.KMEANS: {"k": 3, "max_iterations": 100},
        }
...
class ModelSelector:
    ...
    def compare_algorithms(
        ...
        for algo in algorithms:
            ...
            result = ModelResult(
                ...
                score=score,
                metrics={"accuracy": score, "training_time": training_time},
        ...
        logger.info(
            f"Best algorithm: {results[0].config.algorithm.value} (score: {results[0].score:.4f})"
        )
        return results
    ...
    def _get_default_algorithms(self, model_type: ModelType) -> List[Algorithm]:
        ...
        if model_type == ModelType.CLASSIFICATION:
            ...
            return [Algorithm.LINEAR_REGRESSION, Algorithm.RIDGE, Algorithm.LASSO]
...
class AutoMLPipeline:
    ...
    def auto_train(
        ...
        best_params = self.hyperparameter_tuner.tune_hyperparameters(
            best_result.config.algorithm, max_trials=10
        )

        # Update best model with tuned params
        tuned_score = self.model_selector._train_and_evaluate(
            best_result.config.algorithm, X, y, best_params
        )
        ...
        return {
            ...
            "all_models": [
                {"algorithm": r.config.algorithm.value, "score": round(r.score, 4)}
                for r in results
            ],
