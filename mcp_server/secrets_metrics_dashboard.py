...
class MetricsDashboard:
    ...
    def generate_cli_dashboard(self):
        ...
        # Header
        print("=" * 80)
        print(f"🔍 SECRETS HEALTH DASHBOARD - {self.project.upper()} ({self.context.upper()})")
        print("=" * 80)
        ...
        print(f"  Avg Response Time: {metrics_data.get('avg_response_time_ms', 0):.1f}ms")
        ...
        if timestamp:
            try:
                ...
                print(f"⏰ Last Update: {last_update.strftime('%Y-%m-%d %H:%M:%S')} ({time_diff.total_seconds():.0f}s ago)")
            except ValueError:
    ...
    def generate_charts(self, output_dir: str = "/tmp"):
        ...
        fig.suptitle(f'Secrets Health Metrics - {self.project} ({self.context})', fontsize=16)
        ...
        # Save chart
        chart_file = os.path.join(output_dir, f"secrets_metrics_{self.project}_{self.context}.png")
    ...
    def export_metrics(self, output_file: str, format: str = "json"):
        ...
        if format.lower() == "json":
            ...
            with open(output_file, 'w', newline='') as f:
                ...
                # Write header
                writer.writerow([
                    'timestamp', 'health_score', 'api_score', 'response_time_ms',
                    'total_secrets', 'healthy_secrets', 'warning_secrets', 'critical_secrets'
                ])
...
class WebDashboard:
    ...
    def __init__(self, dashboard: MetricsDashboard, port: int = 5000):
        if not FLASK_AVAILABLE:
            raise ImportError("Flask and Flask-SocketIO not available. Install with: pip install flask flask-socketio")
