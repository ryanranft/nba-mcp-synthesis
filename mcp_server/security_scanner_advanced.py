    ...
    def scan_dependencies(self, dependencies: Dict[str, str]) -> List[Vulnerability]:
        ...
        for package, version in dependencies.items():
            if package in self.known_vulnerabilities:
                for vuln_version, cve, cvss, description in self.known_vulnerabilities[
                    package
                ]:
...
class ComplianceChecker:
    ...
    def check_compliance(
        self, vulnerabilities: List[Vulnerability], standard: str = "OWASP"
    ) -> Dict[str, Any]:
        ...
        compliance_score = max(
            0, 100 - (actual_violations / total_possible_violations * 100)
        )
...
class AdvancedSecurityScanner:
    ...
    def scan_codebase(
        ...
        all_vulnerabilities.extend(
            self.hardening_checker.check_security_headers(http_headers)
        )
        ...
        summary = {
            "critical": sum(
                1 for v in all_vulnerabilities if v.severity == Severity.CRITICAL
            ),
            "high": sum(1 for v in all_vulnerabilities if v.severity == Severity.HIGH),
            "medium": sum(
                1 for v in all_vulnerabilities if v.severity == Severity.MEDIUM
            ),
        ...
        logger.info(
            f"Security scan complete: {len(all_vulnerabilities)} vulnerabilities found"
        )
    ...
    def _get_recommendations(self, scan_result: ScanResult) -> List[str]:
        ...
        if scan_result.summary["critical"] > 0:
            recommendations.append(
                f"🚨 URGENT: Fix {scan_result.summary['critical']} critical vulnerabilities immediately"
            )

        if scan_result.summary["high"] > 0:
            recommendations.append(
                f"⚠️ Address {scan_result.summary['high']} high-severity vulnerabilities within 7 days"
            )

        if scan_result.compliance_score < 80:
            recommendations.append(
                f"📊 Improve compliance score from {scan_result.compliance_score}% to at least 80%"
            )
...
def render_profile(username):
    ...
    return innerHTML = html
        """,
        "auth.py": """
API_KEY = "sk-1234567890abcdef"  # pragma: allowlist secret - test example
password = "admin123"  # pragma: allowlist secret - test example
        """,
    }

    # Dependencies
    dependencies = {"requests": "2.25.0", "flask": "2.0.0", "pillow": "8.3.0"}

    # Config
    config = {"force_https": False, "db_ssl": False}
