...
class ABTestManager:
    ...
    def get_variant_for_user(self, test_name: str, user_id: str) -> Optional[Variant]:
        ...
        if test.status != "running":
            logger.warning(
                f"⚠️  Test {test_name} is not running (status: {test.status})"
            )
            return None
...
if __name__ == "__main__":
    ...
    test = manager.create_test(
        ...
        variants=[
            {"name": "control", "model_version": "v1.0", "traffic_percentage": 0.5},
            {"name": "treatment", "model_version": "v2.0", "traffic_percentage": 0.5},
        ],
        duration_days=7,
    )
    ...
    for i in range(100):
        ...
        manager.record_metric(
            "model_comparison_v1_v2", variant.name, "accuracy", accuracy
        )
