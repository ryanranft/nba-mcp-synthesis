...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler(log_dir / "daily_sync.log"), logging.StreamHandler()],
)
...
class DailyDataSync:
    ...
    def sync_date(self, target_date: datetime):
        ...
        completed_games = [
            g for g in games_list if g["status"] in ["STATUS_FINAL", "final"]
        ]
        ...
        for i, game_id in enumerate(game_ids, 1):
            ...
            try:
                ...
                game_info = next(
                    (g for g in completed_games if g["game_id"] == game_id), {}
                )
...
def main():
    ...
    parser.add_argument(
        "--date", help="Date to sync (YYYY-MM-DD), defaults to yesterday"
    )
    parser.add_argument(
