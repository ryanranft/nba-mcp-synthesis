#!/usr/bin/env python3
"""
Convert Sports Analytics PDFs to Text Format (PyMuPDF Version)

This script converts the 3 sports analytics PDFs from S3 to text format
using PyMuPDF for better handling of encrypted/DRM-protected PDFs.
...
def extract_pdf_text_with_ocr_fallback(book_manager: BookManager, pdf_s3_path: str) -> Optional[str]:
    ...
    try:
        ...
        logger.info(f"🔄 Trying OCR fallback for: {pdf_s3_path}")
        ...
        logger.info(f"🔍 Using OCR on {total_pages} pages...")
        ...
        logger.info(f"✅ OCR extraction completed ({len(text_content)} characters)")
        return text_content.strip()

    except ImportError:
        logger.error("❌ OCR dependencies not available (pytesseract, PIL)")
        return None
    except Exception as e:
...
def convert_book_to_text(book_manager: BookManager, book: Dict) -> bool:
    ...
    # Try PyMuPDF first
    text_content = extract_pdf_text_with_pymupdf(book_manager, book["pdf_s3_path"])
...
def main():
    ...
    logger.info("📚 Sports Analytics PDF to Text Converter (PyMuPDF)")
    logger.info("=" * 60)

    # Check dependencies
    if not check_pymupdf_dependencies():
