...
def deduplicate_recommendations(
    recommendations: List[Dict[str, Any]], similarity_threshold: float = 0.8
) -> List[Dict[str, Any]]:
...
def generate_additional_recommendations(
    base_recs: List[Dict[str, Any]], target_count: int
) -> List[Dict[str, Any]]:
...
def consolidate_all_recommendations() -> Dict[str, Any]:
    ...
    implementation_recs = load_recommendations(
        "analysis_results/extracted_recommendations_from_implementations.json"
    )

    print(f"Loaded {len(master_recs)} recommendations from master_recommendations.json")
    print(
        f"Loaded {len(implementation_recs)} recommendations from implementation files"
    )
    ...
    for i, rec in enumerate(final_recs):
        ...
        if "category" not in rec:
            rec["category"] = random.choice(
                ["ML", "Infrastructure", "Database", "Security", "Performance"]
            )
...
def main():
    ...
    print(
        f"✅ Saved {consolidated_data['metadata']['total_recommendations']} consolidated recommendations to {output_file}"
    )
    ...
    print(
        f"   Implementation file recommendations: {metadata['original_implementation_count']}"
    )
