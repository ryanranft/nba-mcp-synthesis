...
class WorkflowMonitor:
    ...
    def _get_log_tail(self, num_lines: int = 20) -> Dict:
        ...
        log_files = list(logs_dir.glob("overnight_convergence_*.log"))
        if not log_files:
            return {"log_file": None, "lines": [], "error": "No log files found"}
        ...
        try:
            with open(latest_log, "r") as f:
                ...
                tail_lines = (
                    all_lines[-num_lines:] if len(all_lines) > num_lines else all_lines
                )
            ...
            return {"log_file": latest_log.name, "lines": [], "error": str(e)}
