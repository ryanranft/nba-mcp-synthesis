...
class SmartBookDiscovery:
    ...
    BOOK_KEYWORDS = [
        'chapter', 'preface', 'introduction', 'textbook',
        'edition', 'copyright', 'published', 'isbn'
    ]
        ...
        try:
            ...
            for page in pages:
                ...
                for obj in page['Contents']:
                    ...
                    logger.info(f"  📚 Discovered: {book_title} ({category}, {confidence:.0%} confidence)")
    ...
    def add_to_config(
        ...
        for book in discovered_books:
            ...
            if book.confidence < confidence_threshold:
                logger.warning(f"  ⚠️  Low confidence ({book.confidence:.0%}): {book.title}")
    ...
    def generate_discovery_report(
        ...
        for category, books in sorted(by_category.items()):
            report_lines.append(f"### {category.replace('_', ' ').title()} ({len(books)} books)\n")
        ...
        report_lines.append(f"- **High Confidence (≥80%)**: {len(high_confidence)} books")
        report_lines.append(f"  - *Recommendation*: Auto-add to configuration")
        report_lines.append(f"- **Medium Confidence (50-80%)**: {len(medium_confidence)} books")
...
async def main():
    ...
    parser.add_argument(
        '--scan-repos',
        action='store_true',
        help='Scan GitHub repositories for books'
    )
    parser.add_argument(
        '--auto-add',
        action='store_true',
