...
class TestValidationTools(unittest.TestCase):
    async def test_validation_tools(self):
        ...
        try:
            ...
            self.assertTrue(result['success'], f"formula_validate failed: {result.get('error', 'Unknown error')}")
            self.assertIsNotNone(result['report'])
            self.assertIsNotNone(result['overall_status'])
            self.assertIsNotNone(result['overall_score'])
            logger.info(f"Validation completed: {result['overall_status']} (score: {result['overall_score']:.2f})")
            ...
            result = result_tuple[1]
            self.assertTrue(result['success'], f"formula_validate (PER) failed: {result.get('error', 'Unknown error')}")
            self.assertIsNotNone(result['report'])
            logger.info(f"PER validation completed: {result['overall_status']} (score: {result['overall_score']:.2f})")
            ...
            result = result_tuple[1]
            self.assertTrue(result['success'], f"formula_validate (domain-specific) failed: {result.get('error', 'Unknown error')}")
            self.assertIsNotNone(result['report'])
            logger.info(f"Domain-specific validation completed: {result['overall_status']} (score: {result['overall_score']:.2f})")
            ...
            result_tuple = await nba_mcp_server.call_tool(
                "formula_add_reference",
                {"params": reference_params}
            )
            result = result_tuple[1]
            self.assertTrue(result['success'], f"formula_add_reference failed: {result.get('error', 'Unknown error')}")
            ...
            result_tuple = await nba_mcp_server.call_tool(
                "formula_get_references",
                {"params": {}}
            )
            result = result_tuple[1]
            self.assertTrue(result['success'], f"formula_get_references failed: {result.get('error', 'Unknown error')}")
            ...
            result_tuple = await nba_mcp_server.call_tool(
                "formula_compare_validations",
                {"params": comparison_params}
            )
            result = result_tuple[1]
            self.assertTrue(result['success'], f"formula_compare_validations failed: {result.get('error', 'Unknown error')}")
            self.assertIsNotNone(result['comparison'])
            self.assertIsNotNone(result['results'])
            logger.info(f"Compared {len(result['results'])} formulas for {result['comparison_type']}")
            ...
            result_tuple = await nba_mcp_server.call_tool(
                "formula_get_validation_rules",
                {"params": rules_params}
            )
            result = result_tuple[1]
            self.assertTrue(result['success'], f"formula_get_validation_rules failed: {result.get('error', 'Unknown error')}")
            ...
            result = result_tuple[1]
            self.assertTrue(result['success'], f"formula_validate (performance) failed: {result.get('error', 'Unknown error')}")
            self.assertIsNotNone(result['report'])
            logger.info(f"Performance validation completed: {result['overall_status']} (score: {result['overall_score']:.2f})")
            ...
            result = result_tuple[1]
            self.assertTrue(result['success'], f"formula_validate (cross-reference) failed: {result.get('error', 'Unknown error')}")
            self.assertIsNotNone(result['report'])
            logger.info(f"Cross-reference validation completed: {result['overall_status']} (score: {result['overall_score']:.2f})")
            ...
            self.assertTrue(result['success'], f"formula_validate (invalid) failed: {result.get('error', 'Unknown error')}")
            self.assertIsNotNone(result['report'])
            # Should have error status for invalid formula
            self.assertIn(result['overall_status'], ['error', 'warning'])
            logger.info(f"Invalid formula validation completed: {result['overall_status']} (score: {result['overall_score']:.2f})")
