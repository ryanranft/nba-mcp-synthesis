...
def test_clustering():
    ...
    result = ml_clustering_helper.kmeans_clustering(
        data=player_stats, k=3, random_seed=42
    )

    print(
        f"✓ Clustered {len(player_stats)} players into {len(result['centroids'])} groups"
    )
    print(f"✓ Converged: {result['converged']} (iterations: {result['iterations']})")
    print(f"✓ Cluster centroids:")
    for i, centroid in enumerate(result["centroids"]):
        print(
            f"  Cluster {i}: PPG={centroid[0]:.1f}, APG={centroid[1]:.1f}, RPG={centroid[2]:.1f}"
        )
...
def test_classification():
    ...
    model = ml_classification_helper.logistic_regression(
        X_train=X_train, y_train=y_train, learning_rate=0.1, max_iterations=200
    )
    ...
    predictions = ml_classification_helper.logistic_predict(
        X=X_test, weights=model["weights"], return_probabilities=True
    )

    print(f"✓ Predictions made:")
    for i, (pred, prob) in enumerate(
        zip(predictions["predictions"], predictions["probabilities"])
    ):
...
def test_anomaly_detection():
    ...
    result = ml_anomaly_helper.isolation_forest(
        data=data, n_trees=10, contamination=0.2, random_seed=42
    )
    ...
    for anomaly in result["anomalies"]:
        print(
            f"  Player {anomaly['index']}: stats={anomaly['data']}, score={anomaly['score']:.3f}"
        )
...
def test_feature_engineering():
    ...
    result = ml_feature_helper.normalize_features(
        data=raw_stats, method="min-max", feature_range=(0, 1)
    )

    print(
        f"✓ Normalized {result['num_samples']} samples with {result['num_features']} features"
    )
...
def test_knn():
    ...
    result = ml_clustering_helper.find_nearest_neighbors(
        point=test_player, data=train_data, labels=positions, k=3
    )

    print(
        f"✓ Test player stats: Height={test_player[0]}cm, Weight={test_player[1]}kg, PPG={test_player[2]}"
    )
    print(
        f"✓ Predicted position: {result['prediction']} (confidence: {result['confidence']:.1%})"
    )
    print(f"✓ K-nearest neighbors:")
    for i, neighbor in enumerate(result["neighbors"]):
        print(
            f"  {i+1}. Position={neighbor['label']}, Distance={neighbor['distance']:.2f}"
        )
