...
class Phase3ConsolidationBasic:
    ...
    def __init__(
        ...
        self.checkpoint_mgr = CheckpointManager(
            phase="phase_3", save_interval_seconds=300
        )  # 5 minutes
    ...
    async def consolidate_recommendations(self, dry_run: bool = False) -> Dict:
        ...
        for idx, result_file in enumerate(result_files, 1):
            try:
                ...
                book_title = data.get(
                    "book_title", result_file.stem.replace("_convergence_tracker", "")
                )
        ...
        self.checkpoint_mgr.save_checkpoint(
            ...
            state={"complete": True, "consolidated": consolidated},
            force=True,
        )
...
"""
        ...
        for i, book in enumerate(consolidated["metadata"]["books_analyzed"], 1):
            book_recs = [
                r
                for r in consolidated["recommendations"]
                if r.get("source_book") == book
            ]
