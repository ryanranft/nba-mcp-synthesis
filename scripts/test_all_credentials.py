...
class CredentialTester:
    ...
    def log_result(self, category: str, test_name: str, success: bool, message: str = "", error: str = ""):
        """Log test result"""
    ...
    def test_deepseek_api(self):
        ...
        try:
            api_key = get_hierarchical_env("DEEPSEEK_API_KEY", "NBA_MCP_SYNTHESIS", "WORKFLOW")
            if not api_key:
            ...
            if api_key.startswith('sk-'):
                self.log_result("ai_models", "DeepSeek API", True, "API key format is valid")
            else:
                self.log_result("ai_models", "DeepSeek API", False, "API key format may be invalid")

        except Exception as e:
            self.log_result("ai_models", "DeepSeek API", False, "API validation failed", str(e))
    ...
    def test_anthropic_api(self):
        ...
        try:
            ...
            if not api_key:
                self.log_result("ai_models", "Anthropic Claude API", False, "API key not found")
                return

            # Quick format validation instead of API call
            if api_key.startswith('sk-ant-'):
                self.log_result("ai_models", "Anthropic Claude API", True, "API key format is valid")
            else:
                self.log_result("ai_models", "Anthropic Claude API", False, "API key format may be invalid")

        except Exception as e:
            self.log_result("ai_models", "Anthropic Claude API", False, "API validation failed", str(e))
    ...
    def test_openai_api(self):
        ...
        try:
            ...
            if not api_key:
                self.log_result("ai_models", "OpenAI GPT-4 API", False, "API key not found")
                return

            # Quick format validation instead of API call
            if api_key.startswith('sk-proj-'):
                self.log_result("ai_models", "OpenAI GPT-4 API", True, "API key format is valid")
            else:
                self.log_result("ai_models", "OpenAI GPT-4 API", False, "API key format may be invalid")

        except Exception as e:
            self.log_result("ai_models", "OpenAI GPT-4 API", False, "API validation failed", str(e))
    ...
    def test_google_api(self):
        ...
        try:
            ...
            if not api_key:
                self.log_result("ai_models", "Google Gemini API", False, "API key not found")
                return

            # Quick format validation instead of API call
            if api_key.startswith('AIza'):
                self.log_result("ai_models", "Google Gemini API", True, "API key format is valid")
            else:
                self.log_result("ai_models", "Google Gemini API", False, "API key format may be invalid")

        except Exception as e:
            self.log_result("ai_models", "Google Gemini API", False, "API validation failed", str(e))

    def test_ollama_api(self):
        """Test Ollama local API with quick validation"""
        try:
            if not OLLAMA_AVAILABLE:
                self.log_result("ai_models", "Ollama API", False, "Ollama module not installed")
                return

            host = get_hierarchical_env("OLLAMA_HOST", "NBA_MCP_SYNTHESIS", "WORKFLOW") or "http://localhost:11434"
            model = get_hierarchical_env("OLLAMA_MODEL", "NBA_MCP_SYNTHESIS", "WORKFLOW") or "qwen2.5-coder:32b"
            ...
            if response.status_code == 200:
                ...
                if model in model_names:
                    self.log_result("ai_models", "Ollama API", True, f"Model {model} is available")
                else:
                    self.log_result("ai_models", "Ollama API", False, f"Model {model} not found. Available: {model_names}")
            else:
                self.log_result("ai_models", "Ollama API", False, f"Connection failed: {response.status_code}")

        except requests.exceptions.ConnectionError:
            self.log_result("ai_models", "Ollama API", False, "Ollama server not running or not accessible")
        except Exception as e:
            self.log_result("ai_models", "Ollama API", False, "API validation failed", str(e))

    def test_aws_credentials(self):
        """Test AWS credentials"""
        try:
            access_key = get_hierarchical_env("AWS_ACCESS_KEY_ID", "NBA_MCP_SYNTHESIS", "WORKFLOW")
            secret_key = get_hierarchical_env("AWS_SECRET_ACCESS_KEY", "NBA_MCP_SYNTHESIS", "WORKFLOW")
            region = get_hierarchical_env("AWS_REGION", "NBA_MCP_SYNTHESIS", "WORKFLOW") or "us-east-1"

            if not access_key or not secret_key:
                self.log_result("aws_services", "AWS Credentials", False, "Access key or secret key not found")
                return
            ...
            self.log_result("aws_services", "AWS Credentials", True, f"Identity: {identity.get('Arn', 'Unknown')}")

        except Exception as e:
            self.log_result("aws_services", "AWS Credentials", False, "Credential verification failed", str(e))
    ...
    def test_s3_buckets(self):
        ...
        try:
            ...
            # Test primary bucket
            primary_bucket = get_hierarchical_env("S3_BUCKET", "NBA_MCP_SYNTHESIS", "WORKFLOW") or "nba-mcp-books-20251011"
            self._test_s3_bucket(s3_client, primary_bucket, "Primary S3 Bucket")

            # Test books bucket
            books_bucket = get_hierarchical_env("S3_BOOKS_BUCKET", "NBA_MCP_SYNTHESIS", "WORKFLOW") or "nba-mcp-books-20251011"
            if books_bucket != primary_bucket:  # Only test if different
                self._test_s3_bucket(s3_client, books_bucket, "Books S3 Bucket")

        except Exception as e:
            self.log_result("aws_services", "S3 Buckets", False, "S3 testing failed", str(e))
    ...
    def _test_s3_bucket(self, s3_client, bucket_name: str, test_name: str):
        ...
        try:
            ...
            test_content = f"Credential test file created at {datetime.now().isoformat()}"
            ...
            self.log_result("aws_services", test_name, True, f"Full access verified. Objects: {object_count}")

        except Exception as e:
            self.log_result("aws_services", test_name, False, f"Bucket access failed", str(e))
    ...
    def test_aws_glue(self):
        ...
        try:
            ...
            database_name = get_hierarchical_env("GLUE_DATABASE", "NBA_MCP_SYNTHESIS", "WORKFLOW") or "nba_raw_data"

            try:
                response = glue_client.get_database(Name=database_name)
                self.log_result("aws_services", "AWS Glue Database", True, f"Database '{database_name}' accessible")
            except glue_client.exceptions.EntityNotFoundException:
                # Try to list databases
                response = glue_client.get_databases()
                databases = [db['Name'] for db in response['DatabaseList']]
                self.log_result("aws_services", "AWS Glue Database", False, f"Database '{database_name}' not found. Available: {databases}")

            # Test table listing
            try:
                response = glue_client.get_tables(DatabaseName=database_name)
                table_count = len(response.get('TableList', []))
                self.log_result("aws_services", "AWS Glue Tables", True, f"Found {table_count} tables")
            except Exception as e:
                self.log_result("aws_services", "AWS Glue Tables", False, "Table listing failed", str(e))

        except Exception as e:
            self.log_result("aws_services", "AWS Glue", False, "Glue access failed", str(e))

    def test_database_connections(self):
        """Test all database connection variants"""
        # Test RDS_* variables
        self._test_db_connection("RDS", {
        ...
        # Test DB_* variables
        self._test_db_connection("DB", {
            "host": get_hierarchical_env("DB_HOST", "NBA_MCP_SYNTHESIS", "WORKFLOW"),
            "port": get_hierarchical_env("DB_PORT", "NBA_MCP_SYNTHESIS", "WORKFLOW") or "5432",
        ...
        # Test DATABASE_URL
        database_url = get_hierarchical_env("DATABASE_URL", "NBA_MCP_SYNTHESIS", "WORKFLOW")
        if database_url:
    ...
    def _test_db_connection(self, prefix: str, config: Dict[str, str]):
        ...
        try:
            if not all([config["host"], config["database"], config["user"], config["password"]]):
                self.log_result("database", f"{prefix} Variables", False, "Missing required connection parameters")
                return
            ...
            # Test insert
            cursor.execute(f"INSERT INTO {test_table} (test_data) VALUES ('Credential test data');")
            ...
            self.log_result("database", f"{prefix} Variables", True, f"Full access verified. PostgreSQL version: {version[:50]}...")

        except Exception as e:
            self.log_result("database", f"{prefix} Variables", False, "Database connection failed", str(e))
    ...
    def _test_db_url_connection(self, test_name: str, database_url: str):
        ...
        try:
            ...
            self.log_result("database", test_name, True, f"Full access verified. PostgreSQL version: {version[:50]}...")

        except Exception as e:
            self.log_result("database", test_name, False, "Database URL connection failed", str(e))
    ...
    def test_slack_webhook(self):
        ...
        try:
            ...
            if not webhook_url:
                self.log_result("integrations", "Slack Webhook", False, "Webhook URL not found")
                return
            ...
            if response.status_code == 200:
                self.log_result("integrations", "Slack Webhook", True, "Test message sent successfully")
            else:
                self.log_result("integrations", "Slack Webhook", False, f"HTTP {response.status_code}: {response.text}")

        except Exception as e:
            self.log_result("integrations", "Slack Webhook", False, "Webhook test failed", str(e))

    def test_great_expectations_s3(self):
        """Test Great Expectations S3 access"""
        try:
            gx_bucket = get_hierarchical_env("GX_S3_BUCKET", "NBA_MCP_SYNTHESIS", "WORKFLOW")
            if not gx_bucket:
                self.log_result("integrations", "Great Expectations S3", False, "GX_S3_BUCKET not configured")
                return
            ...
            self.log_result("integrations", "Great Expectations S3", True, f"Bucket '{gx_bucket}' accessible")

        except Exception as e:
            self.log_result("integrations", "Great Expectations S3", False, "GX S3 access failed", str(e))
