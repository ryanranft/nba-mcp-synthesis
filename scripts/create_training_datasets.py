...
def create_datasets():
    ...
    print(
        f"Features: {len(baseline_cols) - len(metadata_cols)} ({len(baseline_cols)} total with metadata)"
    )
    ...
    print(
        f"Features: {len(selected_cols) - len(metadata_cols)} ({len(selected_cols)} total with metadata)"
    )
    ...
    print(
        f"Features: {len(df.columns) - len(metadata_cols)} ({len(df.columns)} total with metadata)"
    )
    print(f"✓ Saved to {output_path}")
    ...
    print("=" * 80)
    print(
        f"Baseline:  {len(baseline_cols) - len(metadata_cols)} features → data/baseline_features.csv"
    )
    print(
        f"Selected:  {len(selected_cols) - len(metadata_cols)} features → data/selected_features.csv"
    )
    print(
        f"Full:      {len(df.columns) - len(metadata_cols)} features → data/full_features.csv"
    )
