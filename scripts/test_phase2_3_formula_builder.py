...
async def test_formula_builder_tools():
    ...
    try:
        ...
        ), f"Should have suggestions: {result['suggestions']}"
        logger.info(
            f"Got {result['suggestion_count']} suggestions: {result['suggestions'][:3]}..."
        )
        ...
        result_tuple = await nba_mcp_server.call_tool(
            ...
            {
                "params": {
                    "partial_formula": "(FGM * 85.910 + STL * 53.897) /",
        ...
        logger.info(
            f"Got {result['suggestion_count']} suggestions for PER: {result['suggestions'][:3]}..."
        )
        ...
        result_tuple = await nba_mcp_server.call_tool(
            "formula_builder_preview",
            {
                "params": {
                    "formula": "PTS / (2 * (FGA + 0.44 * FTA))",
                    "variable_values": {"PTS": 25, "FGA": 15, "FTA": 6},
                }
            },
        )
        result = result_tuple[1]
        ...
        ), f"Should have simplified form: {result['simplified']}"
        assert (
            result["calculated_value"] is not None
        ), f"Should have calculated value: {result['calculated_value']}"
        assert (
            len(result["variables"]) > 0
        ), f"Should have variables: {result['variables']}"
        ...
        assert result["latex"] is not None, f"Should have LaTeX: {result['latex']}"
        assert (
            result["calculated_value"] is None
        ), f"Should not have calculated value without inputs"
        ...
        assert result[
            "success"
        ], f"formula_builder_get_templates failed: {result['error']}"
        assert (
            result["template_count"] > 0
        ), f"Should have templates: {result['templates']}"
        ...
        assert (
            result["template_count"] > 0
        ), f"Should have TS% template: {result['templates']}"
        ...
        assert result[
            "success"
        ], f"formula_builder_create_from_template failed: {result['error']}"
        assert (
            result["result"] is not None
        ), f"Should have calculated result: {result['result']}"
        assert (
            result["substituted_formula"] is not None
        ), f"Should have substituted formula: {result['substituted_formula']}"
        ...
        assert (
            result["exported_content"] is not None
        ), f"Should have exported content: {result['exported_content']}"
