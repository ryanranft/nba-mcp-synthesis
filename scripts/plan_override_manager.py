...
class PlanOverrideManager:
    ...
    def analyze_plan_conflicts(
        self, phase_num: int, new_recommendations: List[Dict[str, Any]]
        ...
        logger.info(
            f"Phase {phase_num} analysis: {len(conflicts)} conflicts, {len(enhancements)} enhancements, {len(new_additions)} new additions"
        )
    ...
    def propose_plan_updates(
        self, phase_num: int, analysis: Dict[str, List[Dict[str, Any]]]
    ) -> Dict[str, Any]:
        """
        ...
        proposal = {
            ...
            "safe_updates_count": len(analysis["enhancements"])
            + len(analysis["new_additions"]),
            "conflicts_count": len(analysis["conflicts"]),
        }

        # Write proposal to file
        output_path = os.path.join(
            self.simulator_path, f"docs/phases/phase_{phase_num}/PROPOSED_UPDATES.md"
        )
    ...
    def apply_safe_updates(
        self, phase_num: int, proposal: Dict[str, Any]
    ) -> Dict[str, int]:
        """
        ...
        return {"applied": applied_count, "pending": len(proposal["conflicts"])}
            ...
            if os.path.exists(plan_path):
                try:
                    with open(plan_path, "r") as f:
                        content = f.read()
                    return {"file_path": plan_path, "content": content, "exists": True}
                except Exception as e:
                    logger.warning(f"Error reading plan file {plan_path}: {e}")

        return {"file_path": None, "content": "", "exists": False}

    def _analyze_recommendation(
        self, rec: Dict[str, Any], existing_plan: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
    ...
    def _enhance_plan(self, phase_num: int, rec: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Enhanced Phase {phase_num} plan with: {rec.get('title', 'Untitled')}"
            )
            return True
        ...
        try:
            ...
            if not plan.get("exists", False):
                ...
                plan_dir = os.path.join(
                    self.simulator_path, f"docs/phases/phase_{phase_num}"
                )
...
"""
            ...
            logger.info(
                f"Added to Phase {phase_num} plan: {rec.get('title', 'Untitled')}"
            )
            return True
...
"""
    ...
    print(
        f"  Analysis: {len(analysis['conflicts'])} conflicts, {len(analysis['enhancements'])} enhancements, {len(analysis['new_additions'])} new"
    )

    # Propose updates
    proposal = manager.propose_plan_updates(5, analysis)
    print(
        f"  Proposal: {proposal['safe_updates_count']} safe updates, {proposal['conflicts_count']} conflicts"
    )
