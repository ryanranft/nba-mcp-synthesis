...
class ImmediateStatusChecker:
            ...
            try:
                ...
                for target in self.target_processes:
                    if target in cmdline:
                        ...
                        print(
                            f"📊 Found: {target} (PID: {proc.info['pid']}, Runtime: {runtime:.1f}s)"
                        )
                        break
    ...
    def validate_timestamps(self, processes: List[ProcessStatus]) -> Dict[str, Any]:
        ...
        for process in processes:
            ...
            if process.runtime_seconds > process.expected_max_runtime:
                issues.append(
                    f"Process {process.name} (PID {process.pid}) running for {process.runtime_seconds:.1f}s (expected max: {process.expected_max_runtime}s)"
                )

            # Check if start time is reasonable (not in the future)
            if process.start_time > current_time:
                issues.append(
                    f"Process {process.name} (PID {process.pid}) has future start time: {process.start_time}"
                )

            # Check if start time is too old (more than 24 hours)
            if current_time - process.start_time > 86400:  # 24 hours
                issues.append(
                    f"Process {process.name} (PID {process.pid}) has been running for over 24 hours"
                )
        ...
        return {
            ...
            "stuck_processes": len(
                [p for p in processes if p.runtime_seconds > p.expected_max_runtime]
            ),
        }
    ...
    def generate_report(self) -> Dict[str, Any]:
        ...
        print(
            f"   Valid API keys: {report['summary']['api_keys_valid']}/{report['summary']['api_keys_total']}"
        )
...
def main():
    ...
    # Save report
    report_file = (
        f"logs/immediate_status_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    )
