...
class TestComparisonTools(unittest.TestCase):
    async def test_comparison_tools(self):
        ...
        """
        logger.info(
            "Starting tests for Phase 3.4 Multi-Book Formula Comparison Tools..."
        )
        ctx = MockContext()
        ...
        try:
            ...
            result_tuple = await nba_mcp_server.call_tool(
                "formula_compare_versions", {"params": comparison_params}
            )
            result = result_tuple[1]
            self.assertTrue(
                result["success"],
                f"formula_compare_versions failed: {result.get('error', 'Unknown error')}",
            )
            self.assertIsNotNone(result["comparison"])
            self.assertIsNotNone(result["versions"])
            self.assertGreater(
                len(result["versions"]), 1, "Should have multiple versions"
            )
            self.assertIsNotNone(result["overall_similarity"])
            self.assertIsNotNone(result["summary"])
            logger.info(
                f"PER comparison completed: {result['overall_similarity']:.2f} similarity, {len(result['versions'])} versions"
            )
            ...
            result_tuple = await nba_mcp_server.call_tool(
                "formula_compare_versions", {"params": ts_params}
            )
            result = result_tuple[1]
            self.assertTrue(
                result["success"],
                f"formula_compare_versions (TS) failed: {result.get('error', 'Unknown error')}",
            )
            self.assertIsNotNone(result["variations"])
            self.assertGreater(len(result["variations"]), 0, "Should have variations")
            logger.info(
                f"True Shooting comparison: {len(result['variations'])} variations found"
            )

            # Test 3: formula_get_all_versions
            logger.info("\n--- Testing formula_get_all_versions ---")
            versions_params = {"formula_id": "per"}
            result_tuple = await nba_mcp_server.call_tool(
                "formula_get_all_versions", {"params": versions_params}
            )
            result = result_tuple[1]
            self.assertTrue(
                result["success"],
                f"formula_get_all_versions failed: {result.get('error', 'Unknown error')}",
            )
            self.assertIsNotNone(result["versions"])
            ...
            result_tuple = await nba_mcp_server.call_tool(
                "formula_get_evolution", {"params": evolution_params}
            )
            result = result_tuple[1]
            self.assertTrue(
                result["success"],
                f"formula_get_evolution failed: {result.get('error', 'Unknown error')}",
            )
            ...
            result_tuple = await nba_mcp_server.call_tool(
                "formula_get_recommendations", {"params": rec_params}
            )
            result = result_tuple[1]
            self.assertTrue(
                result["success"],
                f"formula_get_recommendations failed: {result.get('error', 'Unknown error')}",
            )
            self.assertIsNotNone(result["recommendations"])
            self.assertGreater(
                len(result["recommendations"]), 0, "Should have recommendations"
            )
            self.assertIsNotNone(result["recommended_version"])
            logger.info(
                f"Generated {len(result['recommendations'])} recommendations for academic context"
            )
            ...
            result_tuple = await nba_mcp_server.call_tool(
                "formula_get_recommendations", {"params": practical_params}
            )
            result = result_tuple[1]
            self.assertTrue(
                result["success"],
                f"formula_get_recommendations (practical) failed: {result.get('error', 'Unknown error')}",
            )
            self.assertIsNotNone(result["recommendations"])
            logger.info(
                f"Generated {len(result['recommendations'])} recommendations for practical context"
            )
            ...
            result_tuple = await nba_mcp_server.call_tool(
                "formula_add_version", {"params": add_version_params}
            )
            result = result_tuple[1]
            self.assertTrue(
                result["success"],
                f"formula_add_version failed: {result.get('error', 'Unknown error')}",
            )
            ...
            result_tuple = await nba_mcp_server.call_tool(
                "formula_compare_versions", {"params": new_comparison_params}
            )
            result = result_tuple[1]
            self.assertTrue(
                result["success"],
                f"formula_compare_versions (after adding version) failed: {result.get('error', 'Unknown error')}",
            )
            self.assertEqual(
                len(result["versions"]),
                3,
                "Should still have 3 versions (added version not persisted across instances)",
            )
            logger.info(
                f"Comparison after add: {len(result['versions'])} versions, {result['overall_similarity']:.2f} similarity"
            )
            ...
            result_tuple = await nba_mcp_server.call_tool(
                "formula_get_evolution", {"params": usage_evolution_params}
            )
            result = result_tuple[1]
            self.assertTrue(
                result["success"],
                f"formula_get_evolution (usage rate) failed: {result.get('error', 'Unknown error')}",
            )
            self.assertIsNotNone(result["timeline"])
            logger.info(
                f"Usage rate evolution: {len(result['timeline'])} versions in timeline"
            )
            ...
            result_tuple = await nba_mcp_server.call_tool(
                "formula_get_recommendations", {"params": no_context_params}
            )
            result = result_tuple[1]
            self.assertTrue(
                result["success"],
                f"formula_get_recommendations (no context) failed: {result.get('error', 'Unknown error')}",
            )
            self.assertIsNotNone(result["recommendations"])
            logger.info(
                f"Generated {len(result['recommendations'])} recommendations without context"
            )
