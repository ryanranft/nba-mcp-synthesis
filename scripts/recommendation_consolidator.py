...
class RecommendationConsolidator:
    ...
    def __init__(self, master_recs_path: str = "analysis_results/master_recommendations.json"):
        self.master_recs_path = master_recs_path
        self.similarity_threshold = 0.8  # 80% similarity threshold (restored to original)
    ...
    def load_master_recommendations(self) -> Dict:
        ...
        if os.path.exists(self.master_recs_path):
            ...
            logger.error(f"Master recommendations file not found: {self.master_recs_path}")
            return {"recommendations": [], "by_category": {}, "by_book": {}, "last_updated": None}
    ...
    def extract_keywords(self, text: str) -> Set[str]:
        ...
        stop_words = {
            ...
            'implement', 'add', 'create', 'build', 'develop', 'use', 'apply', 'enable', 'support',
            'system', 'framework', 'tool', 'method', 'approach', 'technique', 'process', 'workflow'
        }
    ...
    def find_similar_recommendations(self, recommendations: List[Dict]) -> Dict[str, List[str]]:
        """
        ...
        """
        logger.info(f"Finding similar recommendations among {len(recommendations)} items...")
        ...
        for i, rec1 in enumerate(recommendations):
            ...
            if 'id' not in rec1:
                logger.warning(f"Skipping recommendation without ID: {rec1.get('title', 'Unknown')}")
                continue
            ...
            for j, rec2 in enumerate(recommendations[i+1:], i+1):
                # Skip recommendations without ID
                if 'id' not in rec2:
                    logger.warning(f"Skipping recommendation without ID: {rec2.get('title', 'Unknown')}")
                    continue
            ...
            if len(similar_recs) > 1:
                ...
                similar_groups[primary_id] = similar_recs[1:]  # Exclude primary from similar list
    ...
    def merge_recommendations(self, primary_id: str, similar_ids: List[str],
                           all_recommendations: List[Dict]) -> Dict:
        """
        ...
        primary_rec = next((r for r in all_recommendations if r['id'] == primary_id), None)
        if not primary_rec:
        ...
        for rec in similar_recs:
            ...
            if reasoning and reasoning not in reasoning_parts:
                reasoning_parts.append(f"From {', '.join(rec.get('source_books', []))}: {reasoning}")
        ...
        all_priorities.extend([rec.get('category', 'nice_to_have') for rec in similar_recs])
        ...
        logger.info(f"Consolidated {len(similar_ids) + 1} recommendations into {merged['id']}")
    ...
    def consolidate_all(self) -> Dict:
        ...
        for primary_id, similar_ids in similar_groups.items():
            ...
            merged_rec = self.merge_recommendations(primary_id, similar_ids, recommendations)
            if merged_rec:
        ...
        consolidation_summary = {
            ...
            "recommendations_merged": sum(len(ids) + 1 for ids in similar_groups.values()),
            "reduction_percentage": ((original_count - len(consolidated_recs)) / original_count * 100) if original_count > 0 else 0,
        ...
        logger.info(f"Consolidation complete: {original_count} -> {len(consolidated_recs)} recommendations")
        logger.info(f"Consolidated {len(similar_groups)} groups, merged {sum(len(ids) + 1 for ids in similar_groups.values())} recommendations")
