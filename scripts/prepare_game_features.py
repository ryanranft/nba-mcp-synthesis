...
def calculate_rolling_stats(
    team_stats: pd.DataFrame, team_id: int, before_date: datetime, window: int = 10
) -> Dict[str, float]:
    """
    ...
    team_games["fg_pct"] = team_games["field_goals_made"] / team_games[
        "field_goals_attempted"
    ].replace(0, np.nan)
    team_games["three_pt_pct"] = team_games["three_pointers_made"] / team_games[
        "three_pointers_attempted"
    ].replace(0, np.nan)
    team_games["ft_pct"] = team_games["free_throws_made"] / team_games[
        "free_throws_attempted"
    ].replace(0, np.nan)
    team_games["total_rebounds"] = (
...
def calculate_recent_form(
    games: pd.DataFrame, team_id: int, before_date: datetime, window: int = 5
) -> float:
...
def calculate_rest_days(games: pd.DataFrame, team_id: int, game_date: datetime) -> int:
...
def calculate_season_progress(
    games: pd.DataFrame, team_id: int, before_date: datetime
) -> float:
...
def extract_features_for_game(
    game: pd.Series, all_games: pd.DataFrame, all_team_stats: pd.DataFrame
) -> Dict:
...
def main():
    ...
    print(
        f"Date range: {features_df['game_date'].min()} to {features_df['game_date'].max()}"
    )
    print()
