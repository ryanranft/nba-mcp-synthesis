...
def test_precision_recall_f1():
    ...
    print(
        f"  ✓ Binary classification: P={result['precision']:.2f}, R={result['recall']:.2f}, F1={result['f1_score']:.2f}"
    )
...
def test_confusion_matrix():
    ...
    result = ml_evaluation_helper.confusion_matrix(y_true=y_true, y_pred=y_pred)
    ...
    print(
        f"  ✓ Confusion matrix: TP={result['true_positives']}, FP={result['false_positives']}, TN={result['true_negatives']}, FN={result['false_negatives']}"
    )
...
def test_roc_auc_score():
    ...
    result = ml_evaluation_helper.roc_auc_score(
        y_true=y_true, y_scores=y_scores, num_thresholds=10
    )
    ...
    result2 = ml_evaluation_helper.roc_auc_score(
        y_true=y_true, y_scores=y_scores2, num_thresholds=10
    )
...
def test_classification_report():
    ...
    result = ml_evaluation_helper.classification_report(y_true=y_true, y_pred=y_pred)
    ...
    print(
        f"  ✓ Multi-class report: {len(result['per_class'])} classes, accuracy={result['accuracy']:.2f}"
    )
...
def test_log_loss():
    ...
    result = ml_evaluation_helper.log_loss(
        y_true=y_true, y_pred_proba=y_pred_proba, eps=1e-15
    )
    ...
    result2 = ml_evaluation_helper.log_loss(
        y_true=y_true, y_pred_proba=y_pred_proba2, eps=1e-15
    )
...
def test_mse_rmse_mae():
    ...
    result = ml_evaluation_helper.mse_rmse_mae(y_true=y_true, y_pred=y_pred)
    ...
    print(
        f"  ✓ Regression metrics: MSE={result['mse']:.1f}, RMSE={result['rmse']:.1f}, MAE={result['mae']:.1f}"
    )
...
def test_r2_score():
    ...
    result = ml_evaluation_helper.r2_score(y_true=y_true, y_pred=y_pred)
    ...
    result2 = ml_evaluation_helper.r2_score(y_true=y_true, y_pred=y_pred2)
...
def test_k_fold_split():
    ...
    result = ml_validation_helper.k_fold_split(
        n_samples=100, n_folds=5, shuffle=True, random_seed=42
    )
...
def test_stratified_k_fold_split():
    ...
    result = ml_validation_helper.stratified_k_fold_split(
        y=y, n_folds=5, shuffle=True, random_seed=42
    )
    ...
    print(
        f"  ✓ Stratified K-fold: {result['n_folds']} folds, class distribution preserved"
    )
...
def test_cross_validate():
    ...
    result1 = ml_validation_helper.cross_validate(
        X=X1, y=y1, cv_strategy="k-fold", n_folds=5, shuffle=True, random_seed=42
    )
    ...
    result2 = ml_validation_helper.cross_validate(
        X=X2, y=y2, cv_strategy="stratified", n_folds=5, shuffle=True, random_seed=42
    )
...
def test_compare_models():
    ...
    models = [
        ...
        {"name": "Naive Bayes", "predictions": [1, 0, 1, 0, 0, 1, 1, 0]},  # 6/8 correct
        {
            "name": "Random Forest",
    ...
    result = ml_validation_helper.compare_models(
        models=models, y_true=y_true, metrics=["accuracy", "precision", "recall", "f1"]
    )
    ...
    print(
        f"  ✓ Compared {len(models)} models on {len(result['metrics_compared'])} metrics"
    )
...
def test_paired_ttest():
    ...
    result1 = ml_validation_helper.paired_ttest(
        scores_a=scores_a, scores_b=scores_b, alpha=0.05
    )
    ...
    result2 = ml_validation_helper.paired_ttest(
        scores_a=scores_a2, scores_b=scores_b2, alpha=0.05
    )
...
def test_grid_search():
    ...
    result = ml_validation_helper.grid_search(
        param_grid=param_grid, n_combinations=None
    )
    ...
    print(
        f"  ✓ Grid search: {result['total_combinations']} parameter combinations generated"
    )

    # Test with limit
    result2 = ml_validation_helper.grid_search(param_grid=param_grid, n_combinations=5)

    assert result2["combinations_tested"] == 5
    assert len(result2["param_combinations"]) == 5
    print(
        f"  ✓ Limited grid search: {result2['combinations_tested']} combinations (limited from {result2['total_combinations']})"
    )
