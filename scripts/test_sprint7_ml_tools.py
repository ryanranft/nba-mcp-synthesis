...
def test_clustering_tools():
    ...
    runner.test("Hierarchical clustering created 2 clusters",
                hier_result['final_clusters'], 2)
...
def test_classification_tools():
    ...
    runner.test("Logistic regression has correct weight count",
                len(lr_model['weights']), 3)  # 2 features + 1 bias
    ...
    runner.test("Logistic probabilities returned",
                'probabilities' in lr_pred, True)
    ...
    runner.test("Naive Bayes confidence scores",
                len(nb_pred['confidence']), 2)
    ...
    dt_pred = ml_classification_helper.decision_tree_predict(
        X_test, dt_model['tree']
    )
...
def test_anomaly_detection_tools():
    ...
    data = [
        [5, 5], [6, 5], [5, 6], [6, 6], [4, 5],
        [5, 4], [7, 5], [5, 7], [50, 50]  # Outlier
    ]

    # Test 1-3: Z-score outlier detection
    print("\nZ-score Outlier Detection:")
    zscore_result = ml_anomaly_helper.detect_outliers_zscore(
        data, threshold=2.0
    )
    ...
    runner.test("Z-score returned statistics",
                'feature_means' in zscore_result, True)
    ...
    runner.test("Isolation forest detected anomalies",
                iforest_result['anomaly_count'] > 0, True)
    runner.test("Isolation forest returned scores",
                len(iforest_result['anomaly_scores']) == len(data), True)
    runner.test("Isolation forest has threshold",
                'threshold' in iforest_result, True)

    # Test 7-9: Local Outlier Factor
    print("\nLocal Outlier Factor:")
    lof_result = ml_anomaly_helper.local_outlier_factor(
        data, k=3, contamination=0.1
    )
    runner.test("LOF detected anomalies", lof_result['anomaly_count'] > 0, True)
    runner.test("LOF returned scores",
                len(lof_result['lof_scores']) == len(data), True)
    runner.test("LOF interpretation included",
                'interpretation' in lof_result, True)
...
def test_feature_engineering_tools():
    ...
    runner.test("Min-max normalization",
                'normalized_data' in minmax_result, True)
    runner.test("Min-max values in range",
                minmax_result['normalized_data'][0][0], 0.0)  # Min should be 0
    runner.test("Min-max max value",
                minmax_result['normalized_data'][2][0], 1.0)  # Max should be 1

    # Z-score normalization
    zscore_norm = ml_feature_helper.normalize_features(data, method="z-score")
    runner.test("Z-score normalization",
                'normalized_data' in zscore_norm, True)
    ...
    runner.test("Feature importance calculated",
                'importance_scores' in importance_result, True)
    runner.test("Feature ranking provided",
                'feature_ranking' in importance_result, True)
...
def test_nba_use_cases():
    ...
    clusters = ml_clustering_helper.kmeans_clustering(
        player_stats, k=3, random_seed=42
    )
    runner.test("Clustered players into 3 archetypes",
                len(clusters['centroids']), 3)
    ...
    runner.test("Detected outlier game (50 points)",
                outliers['outlier_count'] > 0, True)
    ...
    normalized = ml_feature_helper.normalize_features(
        raw_stats, method="min-max"
    )
    runner.test("Normalized stats for comparison",
                len(normalized['normalized_data']), 3)
    ...
    runner.test("Predicted player position",
                position_pred['prediction'] in ["SG", "SF"], True)
