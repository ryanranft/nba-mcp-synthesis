...
class FileStructureValidator:
    ...
    def _generate_summary(self) -> Dict[str, Any]:
        ...
        summary = {
            "total_files": sum(
                len(files) for files in self.validation_results.values()
            ),
            "valid_files": 0,
            "invalid_files": 0,
            "by_file_type": {},
    ...
    def generate_report(self, validation_results: Dict[str, Any]) -> str:
        ...
        report.append(
            f"**Total Files Validated:** {validation_results['total_validated']}"
        )
        report.append("")
        ...
        report.append(
            f"- **Overall Success Rate:** {summary['valid_files']/summary['total_files']*100:.1f}%"
        )
        report.append("")
...
def main():
    ...
    for file_type, stats in summary["by_file_type"].items():
        print(
            f"  {file_type.replace('_', ' ').title()}: {stats['valid']}/{stats['total']} ({stats['success_rate']:.1f}%)"
        )
