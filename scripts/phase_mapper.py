...
class PhaseMapper:
    ...
    def map_recommendation_to_phase(self, rec: Dict[str, Any]) -> List[int]:
        ...
        if match_scores:
            sorted_phases = sorted(
                match_scores.items(), key=lambda x: x[1], reverse=True
            )
        ...
        if not matches:
            logger.warning(
                f"No phase match found for '{rec['title']}', defaulting to Phase 5"
            )
            matches = [5]
        elif len(matches) > 1:
    ...
    def get_phase_statistics(
        ...
        stats = {
            ...
            "phases_with_recommendations": len(
                [recs for recs in phase_recs.values() if recs]
            ),
            "phase_distribution": {},
