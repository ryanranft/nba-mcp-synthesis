...
def test_data_processing_pipeline():
    ...
    data['efficiency'] = (data['points'] + data['assists'] + data['rebounds']) / (data['minutes'] + 0.1)
...
def test_merge_operations():
    ...
    # Inner merge
    merged_inner = pd.merge(data1, data2, on='player_id', how='inner', suffixes=('_1', '_2'))

    # Left merge
    merged_left = pd.merge(data1, data2, on='player_id', how='left', suffixes=('_1', '_2'))
...
def main():
    ...
    bottlenecks = profiler.identify_bottlenecks(time_threshold_ms=100.0, call_threshold=1)
    ...
    if slowest:
        ...
        for i, func in enumerate(slowest, 1):
            ...
            print(f"     Avg: {func['avg_time_ms']:.2f}ms | Calls: {func['call_count']} | Total: {func['total_time_ms']:.2f}ms")
