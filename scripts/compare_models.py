...
def compare_models():
    ...
    # Reorder columns
    df = df[['Rank', 'Model', 'Features', 'Description', 'Ensemble',
             'Accuracy', 'AUC', 'Brier', 'Log Loss',
             'LR Weight', 'RF Weight', 'XGB Weight']]

    print("\n" + "="*80)
    print("Performance Comparison (Sorted by Accuracy)")
    print("="*80)
    print(df[['Rank', 'Model', 'Features', 'Accuracy', 'AUC', 'Brier']].to_string(index=False))
    ...
    print(f"  Improvement: {(baseline['Accuracy'] - full_weighted['Accuracy']) * 100:+.2f} percentage points")
    print(f"  AUC: {baseline['AUC']:.4f} vs {full_weighted['AUC']:.4f}")
    print(f"  Brier: {baseline['Brier']:.4f} vs {full_weighted['Brier']:.4f}")

    print(f"\n📊 Feature Count Impact:")
    print(f"  Baseline: {baseline['Features']} features → {baseline['Accuracy']:.1%} accuracy")
    print(f"  Selected: {df[df['Model']=='Selected'].iloc[0]['Features']} features → {df[df['Model']=='Selected'].iloc[0]['Accuracy']:.1%} accuracy")
    print(f"  Full: {full_weighted['Features']} features → {full_weighted['Accuracy']:.1%} accuracy")
    ...
    print(f"  Stacking improvement: {(full_stack['Accuracy'] - full_weighted['Accuracy']) * 100:+.2f} percentage points")
    ...
    print(f"Ensemble Weights: LR={winner['LR Weight']:.1f}, RF={winner['RF Weight']:.1f}, XGB={winner['XGB Weight']:.1f}")
    ...
    print(f"  - Stable weights: {winner['LR Weight']:.0%} Logistic, {winner['RF Weight']:.0%} Random Forest")
    print(f"\nNext steps:")
