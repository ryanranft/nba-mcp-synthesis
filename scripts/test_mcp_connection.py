...
async def test_rds_connection(config: MCPConfig) -> dict:
    ...
    try:
        ...
        return {"status": "❌ Failed", "details": str(e), "tables": 0}
...
async def test_s3_connection(config: MCPConfig) -> dict:
    ...
    try:
        ...
        connector = S3Connector(bucket_name=config.s3_bucket, region=config.s3_region)
        ...
        return {"status": "❌ Failed", "details": str(e), "files": 0}
...
async def test_slack_connection(config: MCPConfig) -> dict:
    ...
    try:
        ...
        return {"status": "❌ Failed", "details": str(e), "sent": False}
...
def test_project_paths(config: MCPConfig) -> dict:
    ...
    try:
        ...
        return {"status": "❌ Error", "details": str(e), "files": "N/A"}
...
def test_api_keys() -> dict:
    ...
    keys = {
        "Anthropic": bool(
            get_hierarchical_env("ANTHROPIC_API_KEY", "NBA_MCP_SYNTHESIS", "WORKFLOW")
        ),
        "OpenAI": bool(
            get_hierarchical_env("OPENAI_API_KEY", "NBA_MCP_SYNTHESIS", "WORKFLOW")
        ),
        "Google": bool(
            get_hierarchical_env("GOOGLE_API_KEY", "NBA_MCP_SYNTHESIS", "WORKFLOW")
        ),
        "AWS": bool(
            get_hierarchical_env("AWS_ACCESS_KEY_ID", "NBA_MCP_SYNTHESIS", "WORKFLOW")
        ),
    }
...
async def main():
    ...
    for service, result in results.items():
        ...
        info_value = result.get(
            "files", result.get("tables", result.get("count", "N/A"))
        )
        info_str = str(info_value) if info_value is not None else "N/A"

        table.add_row(service, result["status"], result["details"], info_str)
