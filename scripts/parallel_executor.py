...
class ParallelExecutor:
    ...
    async def parallel_book_analysis(
        ...
        logger.info(f"📚 Analyzing {len(books)} books in parallel ({batch_size} per batch)")
        ...
        for batch_num, batch in enumerate(batches, 1):
            ...
            for i, result in enumerate(results):
                if isinstance(result, Exception):
                    logger.error(f"   ❌ Failed: {batch[i].get('title', 'Unknown')} - {result}")
            ...
            logger.info(f"   ✅ Batch {batch_num} complete: {len(successful)}/{len(batch)} successful ({batch_elapsed:.1f}s)")
        ...
        for group_num, group in enumerate(grouped, 1):
            ...
            for i, plan in enumerate(plans):
                if isinstance(plan, Exception):
                    logger.error(f"   ❌ Failed: {group[i].get('title', 'Unknown')} - {plan}")
            ...
            logger.info(f"   ✅ Group {group_num} complete: {len(successful)}/{len(group)} successful ({group_elapsed:.1f}s)")
            
            if failed:
                logger.warning(f"   ⚠️  {len(failed)} recommendations failed in this group")
...
if __name__ == "__main__":
    ...
    async def test_parallel_execution():
        ...
        print(f"   Time saved: {metrics['time_saved']:.1f}s ({metrics['percent_reduction']:.1f}%)")
