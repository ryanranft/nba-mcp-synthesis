...
class Tier0WorkflowOrchestrator:
    ...
    async def run_phase2(
        ...
        if not dry_run:
            backup_desc = (
                f"Before analyzing {book_title if book_title else 'all books'}"
            )
            backup_id = self.rollback_mgr.create_backup(
                phase="phase_2", description=backup_desc
            )
        ...
        try:
            ...
            result = subprocess.run(
                cmd, capture_output=False, text=True  # Show output in real-time
            )
    ...
    async def run_phase3(self, dry_run: bool = False) -> bool:
        ...
        if not dry_run:
            backup_id = self.rollback_mgr.create_backup(
                phase="phase_3", description="Before consolidation"
            )
            logger.info(f"📦 Backup created: {backup_id}\n")
        ...
        try:
            ...
            if "error" in result:
                logger.error("\n❌ Phase 3 failed")
                if not dry_run:
                    self.status_mgr.fail_phase(
                        "phase_3", result.get("error", "Unknown error")
                    )
                return False
    ...
    async def run_phase3_5(
        self, dry_run: bool = False, enable_ai_modifications: bool = True
    ) -> bool:
        """
        ...
        if not enable_ai_modifications:
            ...
            if not dry_run:
                self.status_mgr.skip_phase(
                    "phase_3_5", "AI modifications disabled by user"
                )
            return True
        ...
        if not dry_run:
            backup_id = self.rollback_mgr.create_backup(
                phase="phase_3_5", description="Before AI plan modifications"
            )
            logger.info(f"📦 Backup created: {backup_id}\n")
    ...
    async def run_phase4(self, dry_run: bool = False) -> bool:
        ...
        if not dry_run:
            backup_id = self.rollback_mgr.create_backup(
                phase="phase_4", description="Before file generation"
            )
            logger.info(f"📦 Backup created: {backup_id}\n")
        ...
        try:
            ...
            if "error" in result:
                ...
                if not dry_run:
                    self.status_mgr.fail_phase(
                        "phase_4", result.get("error", "Unknown error")
                    )
                return False
    ...
    async def run_phase8_5(self, skip_tests: bool = False) -> bool:
        ...
        self.status_mgr.start_phase(
            "phase_8_5", "Phase 8.5: Pre-Integration Validation", skip_prereq_check=True
        )
    ...
    async def run_workflow(
        ...
        if not await self.run_phase3_5(
            dry_run, enable_ai_modifications=not skip_ai_modifications
        ):
        ...
        logger.info("=" * 60)
        self.status_mgr.skip_phase(
            "phase_5", "Dry-run validation - manual phase, skipped in autonomous mode"
        )
        self.status_mgr.skip_phase(
            "phase_6", "Conflict resolution - manual phase, skipped in autonomous mode"
        )
        self.status_mgr.skip_phase(
            "phase_7", "Manual review - manual phase, skipped in autonomous mode"
        )
        self.status_mgr.skip_phase(
            "phase_8", "Implementation - manual phase, skipped in autonomous mode"
        )
        logger.info(
...
""",
    ...
    parser.add_argument(
        "--skip-validation", action="store_true", help="Skip Phase 8.5 validation"
    )
    parser.add_argument(
        "--parallel",
        action="store_true",
