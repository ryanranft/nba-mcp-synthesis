...
class DeploymentValidator:
    ...
    def validate_secrets(self):
        ...
        try:
            ...
            nba_secrets = [
                s for s in response["SecretList"] if "nba-mcp-synthesis" in s["Name"]
            ]
    ...
    def validate_health_endpoints(self):
        ...
        try:
            ...
            port_forward_cmd = [
                "kubectl",
                "port-forward",
                f"service/nba-mcp-synthesis-service",
                "8080:80",
                "-n",
                self.namespace,
            ]
    ...
    def validate_monitoring(self):
        ...
        if exit_code == 0:
            if "Running" in stdout:
                self.add_result(
                    "Prometheus", "PASS", "Prometheus is running", {}, duration_ms
                )
            else:
                self.add_result(
                    "Prometheus",
        ...
        if exit_code == 0:
            if "Running" in stdout:
                self.add_result(
                    "Grafana", "PASS", "Grafana is running", {}, duration_ms
                )
            else:
                self.add_result(
                    "Grafana",
        ...
        if exit_code == 0:
            if "Running" in stdout:
                self.add_result(
                    "Alertmanager", "PASS", "Alertmanager is running", {}, duration_ms
                )
            else:
                self.add_result(
                    "Alertmanager",
    ...
    def generate_report(self) -> ValidationReport:
        ...
        if failed == 0 and warnings == 0:
            ...
            summary = (
                f"Validation failed with {failed} failures and {warnings} warnings"
            )
    ...
    def print_summary(self, report: ValidationReport):
        ...
        print(
            f"{Colors.PURPLE}NBA MCP Synthesis - Deployment Validation Report{Colors.NC}"
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Validate NBA MCP Synthesis deployment"
    )
    parser.add_argument(
        "-v", "--verbose", action="store_true", help="Enable verbose output"
    )
    parser.add_argument(
        "-n", "--namespace", default="nba-mcp-synthesis", help="Kubernetes namespace"
    )
    ...
    parser.add_argument(
        "--skip-secrets", action="store_true", help="Skip secrets validation"
    )
    parser.add_argument(
        "--skip-application", action="store_true", help="Skip application validation"
    )
    parser.add_argument(
        "--skip-monitoring", action="store_true", help="Skip monitoring validation"
    )
    ...
    try:
        print(
            f"{Colors.BLUE}Starting NBA MCP Synthesis deployment validation...{Colors.NC}"
        )
