...
def generate_backtest_predictions(
    features_df: pd.DataFrame, model, calibration_season: str = "2023-24"
):
    """
    ...
    for idx, game in tqdm(
        calibration_games.iterrows(), total=len(calibration_games), desc="Predicting"
    ):
        try:
            # Extract features
            ...
            X = (
                ...
                .fillna(features_df[feature_cols].mean())
                .values
            )
...
def analyze_predictions(predictions_df: pd.DataFrame, output_dir: Path):
    ...
    plt.plot(
        ...
        prob_true,
        "ro-",
        label=f"Model Predictions (Brier: {brier:.4f})",
        linewidth=2,
        markersize=8,
    )
    ...
    plt.figure(figsize=(10, 6))
    plt.hist(
        sim_probs[outcomes == 1], bins=20, alpha=0.6, label="Home Wins", color="blue"
    )
    plt.hist(
        sim_probs[outcomes == 0], bins=20, alpha=0.6, label="Away Wins", color="red"
    )
    plt.xlabel("Predicted Probability (Home Win)", fontsize=12)
    ...
    print(
        f"✓ Saved prediction distribution to {output_dir}/prediction_distribution.png"
    )
    ...
    print(
        f"✓ Saved confidence distribution to {output_dir}/confidence_distribution.png"
    )
...
def validate_data_quality(predictions_df: pd.DataFrame):
    ...
    if len(predictions_df) < 100:
        issues.append(
            f"⚠ Sample size ({len(predictions_df)}) below recommended minimum (100)"
        )
    elif len(predictions_df) < 200:
        issues.append(
            f"⚠ Sample size ({len(predictions_df)}) below ideal minimum (200)"
        )
    ...
    if home_win_rate < 0.4 or home_win_rate > 0.7:
        issues.append(
            f"⚠ Class imbalance detected (home win rate: {home_win_rate:.1%})"
        )
...
def main():
    ...
    # Generate predictions
    predictions_df = generate_backtest_predictions(
        features_df, model, args.calibration_season
    )
    ...
    print(
        f"Data quality: {'✓ PASS' if is_valid else '⚠ WARNING - Review issues above'}"
    )
    print()
    print("Next steps:")
