...
def load_parquet_data(years: List[int]) -> tuple:
    ...
    for year in tqdm(years, desc="Loading years"):
        # Player box scores
        player_file = (
            PARQUET_BASE / f"load_nba_player_box/parquet/nba_data_{year}.parquet"
        )
        ...
        schedule_file = (
            PARQUET_BASE / f"load_nba_schedule/parquet/nba_schedule_{year}.parquet"
        )
    ...
    player_box = (
        pd.concat(player_dfs, ignore_index=True) if player_dfs else pd.DataFrame()
    )
    team_box = pd.concat(team_dfs, ignore_index=True) if team_dfs else pd.DataFrame()
    schedule = (
        pd.concat(schedule_dfs, ignore_index=True) if schedule_dfs else pd.DataFrame()
    )
    ...
    print(
        f"Date Range: {player_box['game_date'].min()} to {player_box['game_date'].max()}"
    )
    print()
...
def extract_completed_games(
    schedule: pd.DataFrame, team_box: pd.DataFrame
) -> pd.DataFrame:
...
def calculate_rolling_stats(
    ...
    team_games = (
        team_box[mask].sort_values("game_date", ascending=False).head(window).copy()
    )
    ...
    fg3a = (
        team_games["three_point_field_goals_attempted"].astype(float).replace(0, np.nan)
    )
...
def calculate_player_features(
    player_box: pd.DataFrame, team_id: int, before_date: pd.Timestamp, window: int = 10
) -> Dict[str, float]:
    ...
    team_games = player_box[
        (player_box["team_id"] == team_id) & (player_box["game_date"] < before_date)
    ].copy()
    ...
    team_games["player_points"] = (
        team_games["fgm"] * 2 + team_games["fg3m"] * 3 + team_games["ftm"]
    )
    ...
    for i in range(min(3, len(player_stats))):
        ...
        features[f"top{i+1}_usage_pct_l{window}"] = (
            float(player["ppg"] / team_ppg) if team_ppg > 0 else 0
        )
...
def calculate_rest_days(
    team_box: pd.DataFrame, team_id: int, game_date: pd.Timestamp
) -> int:
    """Calculate days of rest since last game."""
    previous_games = team_box[
        (team_box["team_id"] == team_id) & (team_box["game_date"] < game_date)
...
def calculate_h2h_stats(
    ...
    home_games = team_box[
        (team_box["team_id"] == home_team_id) & (team_box["game_date"] < before_date)
    ]
    away_games = team_box[
        (team_box["team_id"] == away_team_id) & (team_box["game_date"] < before_date)
    ]
    ...
    h2h_games = team_box[team_box["game_id"].isin(h2h_game_ids)].sort_values(
...
def extract_game_features(
    ...
    for window in [5, 10, 20]:
        home_stats = calculate_rolling_stats(
            team_box, home_team_id, game_date, window=window
        )
    ...
    for window in [5, 10, 20]:
        away_stats = calculate_rolling_stats(
            team_box, away_team_id, game_date, window=window
        )
    ...
    # Home/away splits
    home_home_stats = calculate_rolling_stats(
        team_box, home_team_id, game_date, location="home", window=20
    )
    for key, value in home_home_stats.items():
        if key != "games_played":
            features[f"home_{key}"] = value

    away_away_stats = calculate_rolling_stats(
        team_box, away_team_id, game_date, location="away", window=20
    )
    ...
    home_player_features = calculate_player_features(
        player_box, home_team_id, game_date
    )
    for key, value in home_player_features.items():
        features[f"player__home_{key}"] = value

    away_player_features = calculate_player_features(
        player_box, away_team_id, game_date
    )
    ...
    if "player__home_top5_ppg" in features and "player__away_top5_ppg" in features:
        features["player__top5_ppg_advantage"] = (
            features["player__home_top5_ppg"] - features["player__away_top5_ppg"]
        )
