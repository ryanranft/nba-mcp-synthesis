...
class VersionTracker:
    ...
    def generate_file_header(
        ...
        header.extend(["DO NOT EDIT THIS FILE MANUALLY", '"""', ""])
    ...
    def generate_markdown_header(
        ...
        if models_used:
            header.append(
                "Models: " + ", ".join(f"{k}={v}" for k, v in models_used.items())
            )
    ...
    def add_header_to_file(self, file_path: Path, header: str, backup: bool = True):
        """
    ...
    def get_source_books_metadata(
        self, books_dir: Path, book_names: List[str]
    ) -> List[Dict]:
        """
        ...
        for book_name in book_names:
            ...
            if book_files:
                ...
                metadata.append(
                    {"title": book_name, "hash": book_hash, "path": str(book_path)}
                )
            else:
                metadata.append(
                    {"title": book_name, "hash": "N/A", "path": "Not found"}
                )

        return metadata

    def create_version_manifest(
        self, output_dir: Path, files: List[Path], metadata: Dict
    ):
        """
...
def main():
    ...
    if args.directory:
        ...
        models_used = {"gemini": "gemini-2.0-flash-exp", "claude": "claude-sonnet-4"}
        ...
        models_used = {"gemini": "gemini-2.0-flash-exp", "claude": "claude-sonnet-4"}
        regenerate_command = (
            'python scripts/run_full_workflow.py --book "All Books" --parallel'
        )
