...
class ResilientBookAnalyzer:
    ...
    def __init__(self, project: str = "nba-mcp-synthesis", context: str = "production"):
        ...
        logger.info(
            "✅ ResilientBookAnalyzer initialized with Google and DeepSeek only"
        )
    ...
    async def analyze_book(self, book: Dict[str, Any]) -> ResilientAnalysisResult:
        ...
        logger.info(
            f"🚀 Starting resilient analysis for: {book.get('title', 'Unknown')}"
        )
        ...
        try:
            google_result = await asyncio.wait_for(
                self.google_model.analyze_book_content(
                    book_content=book_content, book_metadata=book
                ),
            ...
            if google_result.get("success", False):
                logger.info(
                    f"✅ Google analysis complete: {len(google_result.get('recommendations', []))} recommendations"
                )
                total_cost += google_result.get("cost", 0.0)
                total_tokens += google_result.get("tokens_input_estimate", 0)
            else:
                logger.warning(
                    f"⚠️ Google analysis failed: {google_result.get('error', 'Unknown error')}"
                )
                google_result = {
                    "recommendations": [],
                    "cost": 0.0,
        ...
        try:
            ...
            if deepseek_result.get("success", False):
                logger.info(
                    f"✅ DeepSeek analysis complete: {len(deepseek_result.get('recommendations', []))} recommendations"
                )
                total_cost += deepseek_result.get("cost", 0.0)
                total_tokens += deepseek_result.get("tokens_input", 0)
            else:
                logger.warning(
                    f"⚠️ DeepSeek analysis failed: {deepseek_result.get('error', 'Unknown error')}"
                )
                deepseek_result = {
                    "recommendations": [],
                    "cost": 0.0,
