...
def main():
    ...
    print("=" * 80)
    print(
        importance_df[["feature", "weighted_avg", "rf_importance", "xgb_importance"]]
        .head(20)
    ...
    print(
        player_features[
            ["feature", "weighted_avg", "rf_importance", "xgb_importance"]
        ].to_string(index=False)
    )
    ...
    analysis_results = {
        ...
        "feature_importance_stats": {
            "player_features_avg_importance": float(
                player_features["weighted_avg"].mean()
            ),
            "non_player_features_avg_importance": float(
                importance_df[
                    ~importance_df["feature"].isin(player_features["feature"])
                ]["weighted_avg"].mean()
            ),
    ...
    print(
        f"Top 10 player features avg importance: {analysis_results['feature_importance_stats']['top10_player_avg_importance']:.6f}"
    )
    print(
        f"Bottom {len(bottom_player)} player features avg importance: {analysis_results['feature_importance_stats']['bottom_player_avg_importance']:.6f}"
    )
    print(
        f"Non-player features avg importance: {analysis_results['feature_importance_stats']['non_player_features_avg_importance']:.6f}"
    )
    print(
        f"\nRecommendation: Remove {len(bottom_player)} low-importance player features"
    )
    print(
        f"New feature count: {len(feature_names)} - {len(bottom_player)} = {len(feature_names) - len(bottom_player)}"
    )
