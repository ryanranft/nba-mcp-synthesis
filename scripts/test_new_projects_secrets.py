...
def test_environment_variables(project, context):
    ...
    for var in expected_vars:
        value = get_hierarchical_env(var.split('_')[0] + '_' + '_'.join(var.split('_')[1:-1]), "NBA_MCP_SYNTHESIS", context)
        if value:
...
def main():
    ...
    print(f"nba_mcp_synthesis_global (development): {'✅ PASS' if success2 else '❌ FAIL'}")
    print(f"nba_mcp_synthesis_global (production): {'✅ PASS' if success3 else '❌ FAIL'}")
