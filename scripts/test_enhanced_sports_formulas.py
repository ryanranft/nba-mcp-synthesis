...
async def test_enhanced_sports_formulas():
    ...
    try:
        ...
        result_tuple = await nba_mcp_server.call_tool(
            "algebra_sports_formula",
            {"params": drb_pct_params}
        )
        result = result_tuple[1]
        logger.info(f"Defensive Rebound Percentage calculation: {result.get('result', 'N/A')}")
        ...
        result_tuple = await nba_mcp_server.call_tool(
            "algebra_sports_formula",
            {"params": shooting_diff_params}
        )
        result = result_tuple[1]
        logger.info(f"Shooting Efficiency Differential calculation: {result.get('result', 'N/A')}")
