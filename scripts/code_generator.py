...
class CodeGenerator:
    ...
    def _generate_python_module(
        self, rec: Dict[str, Any], file_types: List[str]
    ) -> GeneratedFile:
    ...
    def _generate_sql_script(self, rec: Dict[str, Any]) -> GeneratedFile:
        ...
        return GeneratedFile(
            path=f"{filename}.sql", content=content, language="sql", file_type="sql"
        )
    ...
    def _generate_readme(
        self, rec: Dict[str, Any], generated_files: List[GeneratedFile]
    ) -> GeneratedFile:
        ...
        return GeneratedFile(
            path="README.md", content=content, language="markdown", file_type="doc"
        )
    ...
    def save_generated_files(
        self, files: List[GeneratedFile], subdir: Optional[str] = None
    ) -> Path:
        """
...
def main():
    ...
    parser.add_argument(
        "--output-dir", default="generated_code", help="Output directory"
    )
    ...
    for rec in recommendations[:5]:  # Limit to first 5
        try:
            ...
            logger.error(
                f"❌ Failed to generate code for {rec.get('title', 'Unknown')}: {e}"
            )
