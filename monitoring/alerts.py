...
class AlertManager:
    ...
    def send_slack_alert(
        self, rule: AlertRule, value: float, context: Optional[Dict] = None
    ) -> bool:
        """Send alert to Slack"""
        ...
        emoji = (
            ...
            else "⚠️" if rule.severity == "warning" else "ℹ️"
        )
        color = (
            "#FF0000"
            if rule.severity == "critical"
            else "#FFA500" if rule.severity == "warning" else "#0000FF"
        )
    ...
    def send_test_alert(self) -> bool:
        ...
        try:
            response = requests.post(
                self.slack_webhook_url, json=test_message, timeout=10
            )
...
if __name__ == "__main__":
    ...
    if not manager.slack_webhook_url:
        ...
        print(
            "2. Set environment variable: export SLACK_WEBHOOK_URL='your-webhook-url'"
        )
        sys.exit(1)
